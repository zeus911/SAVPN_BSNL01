	...
	<%
		...
		String routing_protocol = ad18.value;
	...
	//End of 15150    
		...
		String preSelectedIPNetAddr = ad37.value;
	...
	if ( "false".equals(request.getParameter("vrrpEnabled"))) {
		...
		// BGP Neighbor
		boolean BGPNeighborCheck = false;
		if (request.getParameter("BGPNeighborCheck") == null) {
			BGPNeighborCheck = false;
		} else {
			BGPNeighborCheck = true;
		}
		String BGPNeighbor = null;
		BGPNeighbor = request.getParameter("BGPNeighbor");
		if (BGPNeighbor == null) {
			BGPNeighbor = "";
		}
		...
		try {
		  ...
		  if ("protection".equals(attachment_type) && null!=original_attachmentid){
			...
			com.hp.ov.activator.cr.inventory.Interface selectedIF = null;
			
			// First check for avalaible interfaces count
			for (int i = 0 ; i < interfaceNames.length; i++) 
			{
				if (InteractionHelper.isValidRatelimit(con, router_id, interfaceNames[i], rateLimitBW)) validInterfaces++;
			}
			
			if (InteractionHelper.isValidRatelimit(con, router_id, interface_name, rateLimitBW))
			{
				selectedIF = com.hp.ov.activator.cr.inventory.Interface.findByNe_idname(con, router_id, interface_name)[0];
				isSharedPortAllowed = ServiceMultiplexExtension.isSharedPortAllowed(con, selectedIF.getTerminationpointid(), router_id, "L3VPN");
			}
			else if (validInterfaces > 0)
			{
				// If the selected interface is not valid, find the first valid interface
				for (int i = 0 ; i < interfaceNames.length; i++) 
				{
					if (InteractionHelper.isValidRatelimit(con, router_id, interfaceNames[i], rateLimitBW))
					{
						selectedIF = com.hp.ov.activator.cr.inventory.Interface.findByNe_idname(con, router_id, interfaceNames[i])[0];
	...
	<%        } else {
				if ( interfaceNames.length != 0) 
				{					
					// Now print interfaces if available
					...
					{
						noInterfaces = true; %>
						<b>No suitable interfaces available in this router</b>
					<%}
				} 
				else
				{%>
					<b>No interfaces available in this router</b>
				<%}
			} %>
			  </td>
			</tr>
				...
				if ("MPLS-PortVlan".equals(service_type) && vlan_id_type.equals("customer_provided")) {
				  ...
				  try {  if(role.equalsIgnoreCase("AccessSwitch") || role.equalsIgnoreCase("AggregationSwitch")) {
							...
							freeVlanIds = peRouterWrapper.getFreeVlanIds(con, interface_name, "Attachment");
														String q = " select ci.vlanid from cr_interface ci inner join cr_terminationpoint tp on tp.terminationpointid=ci.TERMINATIONPOINTID inner join cr_networkelement ne on ne.networkelementid=tp.ne_id where tp.ne_id=? and tp.name=? ";
														
														ArrayList<String> pList=new ArrayList<String>();
														out.println("Vlan range "+ interface_name + " #### "+peRouterWrapper.getNetworkelementid());	
														PreparedStatement statePstmt = null;	
														ResultSet rs=null;
														try {
															  statePstmt = con.prepareStatement(q);
															  statePstmt.setString(1, peRouterWrapper.getNetworkelementid());
															  statePstmt.setString(2, interface_name);
														...
														int cnt=0;
														for (int  vlanId =1;vlanId<=4096; vlanId++) {
														...
														freeVlanIds = new int[newVlanList.size()];
			  ...
			  if ("MPLS-PortVlan".equals(service_type) && vlan_id_type.equals("customer_provided")) {
				try {  if(role.equalsIgnoreCase("AccessSwitch") || role.equalsIgnoreCase("AggregationSwitch")) {
						...
						//  System.out.println("Free vlans" + freeVlanIds.length);
							...
							freeVlanIds = peRouterWrapper.getFreeVlanIds(con, interface_name, "Attachment");
							String q = " select ci.vlanid from cr_interface ci inner join cr_terminationpoint tp on tp.terminationpointid=ci.TERMINATIONPOINTID inner join cr_networkelement ne on ne.networkelementid=tp.ne_id where tp.ne_id=? and tp.name like '"+interface_name+"%'";
														
														ArrayList<String> pList=new ArrayList<String>();
														System.out.println("Vlan range "+ interface_name + " #### "+peRouterWrapper.getNetworkelementid());	
														PreparedStatement statePstmt = null;
														ResultSet rs=null;
														try {
															  statePstmt = con.prepareStatement(q);
				  ...
				  System.out.println("Exception in Interface selection: "  );
				  e.printStackTrace();
				  exceptionFlag = true;
				  service_type ="Port";
	...
	<%      if ("Serial".equals(ri[0].getType())) {
			  ...
			  <% if ("Huawei".equals(vendor) && InteractionHelper.isSONETChildInterface(con, ri[0].getTerminationpointid())) {%>
  ...
  %>
	...
	<tr>
	   ...
	   <%
		...
		{ 
			if(ipnetaddr!=null && ipnetaddr.equals(ipnetAll[i].getIpnetaddr().split("@")[1])) {
	    %>
		<option value="<%=ipnetAll[i].getIpnetaddr()%>" selected><%=ipnetAll[i].getIpnetaddr().split("@")[1]%>			
	    <%
		 }else{
		%>
		<option value="<%=ipnetAll[i].getIpnetaddr()%>"><%=ipnetAll[i].getIpnetaddr().split("@")[1]%></option>
		<% }}%>		
		 </select>
	   </td>
	</tr>	
	...
	<%      if ( "BGP".equals(routing_protocol) ) { %>
	...
	<%      if ("BGP".equals(routing_protocol) && Arrays.asList(BGPCredentialsVendors).contains(vendor) && !vendor.equals("Cisco")) { %>
	<tr>
	   <td><b>Username</b></td>
	   <td>
	...
	<%      if ("BGP".equals(routing_protocol) && Arrays.asList(BGPCredentialsVendors).contains(vendor) && vendor.equals("Cisco") && networkElements[selected].getOsversion().equals("CiscoXR")) { %>
	<tr>  
	   <td><b>Password</b></td>
	   <td>
		 <input type="password" name="BGPPassword" id="BGPPassword" value="<%=BGPPassword %>" size="30">
	...
	<% if ("BGP".equals(routing_protocol) && Arrays.asList(BGPMultiHopVendors).contains(vendor)) { %>
	<tr>
		<td><b>Multi-Hop</b></td>
		<td>
	...
	<%      if (BGPMultiHopCheck) { %>
		<tr>
			<td><b>Override BGP neighbor IP</b></td>
			<td>
				<input TYPE="checkbox" id="BGPNeighborCheck" name="BGPNeighborCheck" VALUE="true" <%= (BGPNeighborCheck) ? " CHECKED":" " %> onClick="document.getElementById('BGPNeighborCheck').value=form.BGPNeighborCheck.checked;document.rsform.submit()">
				<%      if (BGPNeighborCheck) { %>
					<input type="input" name="BGPNeighbor" id="BGPNeighbor" value="<%=BGPNeighbor %>" size="30">
				<%      } %>
			</td>
		</tr>
	<%      } %>
	...
	<%			if (Arrays.asList(BGPMultiHopVendors).contains(vendor)) { %>
					...
					if (document.rsform.BGPNeighbor != null) {
						BGPNeighbor.value=document.rsform.BGPNeighbor.value;
						if (ValidateIPaddress(BGPNeighbor) == false) {
							document.rsform.BGPNeighbor.focus();
							document.rsform.BGPNeighbor.select();
							return false;
						}
					}
	<%			} %>
	...
	//Start -AN
			...
			<input type="hidden" name="BGPNeighbor" value="<%= BGPNeighbor %>">
			<input type="hidden" name="BGPNeighborCheck" value="<%= BGPNeighborCheck %>">
	...
	<script language="JavaScript">    
		...
		{
			if (inputText == null || inputText.value == null || inputText.value == "") {
				alert("You must enter an IP address for BGP neighbor IP");
				return false;
			}
			...
			else
			{
				...
				if(inputText.value.match(ipformat))  
				{
					...
					alert("You have entered an invalid IPv4 or IPv6 address: "+inputText.value);
					return false;	
				}
				alert("You have entered an invalid IP address: "+inputText.value);
				return false;
			}  
		}
