...
<html>
  <head>
    <title><bean:message bundle="igw_qosApplicationResources" key="<%= igw_qosConstants.JSP_DELETE_TITLE %>"/></title>
    ...
    {
      window.document.igw_qosForm.action = '/activator<%=moduleConfig%>/DeleteCommitigw_qosAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.igw_qosForm.submit();
    }
    function init()
    {
      var confirm = new HPSAConfirm('Confirm','<bean:message bundle="igw_qosApplicationResources" key="delete.confirm"/>', '<bean:message bundle="InventoryResources" key="confirm.ok_button.label"/>', '<bean:message bundle="InventoryResources" key="confirm.cancel_button.label"/>');
...
<h2 style="width:100%; text-align:center;">
    <bean:message bundle="igw_qosApplicationResources" key="jsp.delete.title"/>
</h2> 
...
%>
    ...
    <html:form action="/DeleteCommitigw_qosAction.do">
    <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
                  <html:hidden property="qos_id" value="<%= String.valueOf(QOS_ID) %>"/>
              </html:form>
  </body>
</html>
