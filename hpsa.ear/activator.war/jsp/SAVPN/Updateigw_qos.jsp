...
String formAction = "/UpdateCommitigw_qosAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("qos_id") ;
}
...
<html>
  <head>
    ...
    {
      window.document.igw_qosForm.action = '/activator<%=moduleConfig%>/UpdateFormigw_qosAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.igw_qosForm.submit();
    }
    function performCommit()
    {
      window.document.igw_qosForm.action = '/activator<%=moduleConfig%>/UpdateCommitigw_qosAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.igw_qosForm.submit();
    }
    function init()
    {
...
if(beanigw_qos==null)
   beanigw_qos = (com.hp.ov.activator.vpn.inventory.igw_qos) request.getSession().getAttribute(igw_qosConstants.IGW_QOS_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String QOS_ID = beanigw_qos.getQos_id();
        
            
                            
            
                
                String NAME = beanigw_qos.getName();
        
            
                            
            
                
                String COMMANDS = beanigw_qos.getCommands();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="igw_qosApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="qos_id" value="<%= QOS_ID %>"/>
                                                        <html:text disabled="true" property="qos_id" size="24" value="<%= QOS_ID %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="commands" size="24" value="<%= COMMANDS %>"/>
                                                </table:cell>
              <table:cell>
