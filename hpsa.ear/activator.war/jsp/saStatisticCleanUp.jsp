...
<%@ page import="java.net.*,
         ...
         info="Display statistical filter clean up." 
         session="true" 
...
<%!
    //I18N strings
    final static String header          = com.hp.ov.activator.activatorUiNLS.MsgHandler.getString("884", "Statistical clean up filter");
    ...
    final static String confirmMsg      = com.hp.ov.activator.activatorUiNLS.MsgHandler.getString("887", "Are you sure you want to delete all statistic data in range:");
    final static String confirmMsg2     = com.hp.ov.activator.activatorUiNLS.MsgHandler.getString("888", "Are you sure you want to delete all statistic data up to:");
...
<form name="inputForm" action="/activator/jsp/saStatisticCleanUpRedirect.jsp" method="POST" onSubmit="return askConfirm();">
<%
...
<%
  String selHost=statClnFilter.getHostName(), name;
...
<tr>
    ...
    <td nowrap title="<%=MessageFormat.format(beginDateTitle,new Object[]{df.toPattern()})%>">
        <input type="text" size="8" name="beginDate" id='inputBeginDate' 
            title="<%=MessageFormat.format(beginDateTitle,new Object[]{df.toPattern()})%>" value="<jsp:getProperty name="statClnFilter" property="beginDate" />"/>
    ...
    <td title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>">
        <input type="text" size="8" name="beginTime" title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>" value="<jsp:getProperty name="statClnFilter" property="beginTime" />"/></td>
</tr>
<tr>
    ...
    <td nowrap title="<%=MessageFormat.format(endDateTitle,new Object[]{df.toPattern()})%>">
        <input type="text" size="8" name="endDate" id='inputEndDate' title="<%=MessageFormat.format(endDateTitle,new Object[]{df.toPattern()})%>" value="<jsp:getProperty name="statClnFilter" property="endDate" />"/>
        <img src="/activator/images/date_select.gif" id="buttonEndDate" class="dateInputTriger"/>
    </td>
    <td class="filterTextTd" title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>"><%=time%>:</td>
    <td title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>">
        <input type="text" size="8" name="endTime" title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>" value="<jsp:getProperty name="statClnFilter" property="endTime" />"/></td>
</tr>
<tr>
