...
<html>
  <head>
    <title><bean:message bundle="PWPolicyApplicationResources" key="<%= PWPolicyConstants.JSP_DELETE_TITLE %>"/></title>
    ...
    {
      window.document.PWPolicyForm.action = '/activator<%=moduleConfig%>/DeleteCommitPWPolicyAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.PWPolicyForm.submit();
    }
    function init()
    {
      var confirm = new HPSAConfirm('Confirm','<bean:message bundle="PWPolicyApplicationResources" key="delete.confirm"/>', '<bean:message bundle="InventoryResources" key="confirm.ok_button.label"/>', '<bean:message bundle="InventoryResources" key="confirm.cancel_button.label"/>');
...
nfB.setMaximumFractionDigits(6);
                  ...
                  String PWPolicyId = StringFacility.replaceAllByHTMLCharacter(beanPWPolicy.getPwpolicyid());
                        
                                  
                      String Name = StringFacility.replaceAllByHTMLCharacter(beanPWPolicy.getName());
                        
                                  
                      String Description = StringFacility.replaceAllByHTMLCharacter(beanPWPolicy.getDescription());
                        
                                  
                      boolean UsernameEnabled = new Boolean(beanPWPolicy.getUsernameenabled()).booleanValue();
                  
                                  
                      String Username = StringFacility.replaceAllByHTMLCharacter(beanPWPolicy.getUsername());
                        
                                  
                      String Password = StringFacility.replaceAllByHTMLCharacter(beanPWPolicy.getPassword());
                        
                                  
                      String EnablePassword = StringFacility.replaceAllByHTMLCharacter(beanPWPolicy.getEnablepassword());
                        
                                  
    
%>
<h2 style="width:100%; text-align:center;">
    <bean:message bundle="PWPolicyApplicationResources" key="jsp.delete.title"/>
</h2> 
...
%>
    ...
    <html:form action="/DeleteCommitPWPolicyAction.do">
    <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
                  <html:hidden property="pwpolicyid" value="<%= String.valueOf(PWPolicyId) %>"/>
              </html:form>
  </body>
</html>
