...
if(form==null) {
 form=new NetworkElementDataloadDataForm();
} 
    
      
  
String datasource = (String) request.getParameter(NetworkElementDataloadDataConstants.DATASOURCE);
String tabName = (String) request.getParameter(NetworkElementDataloadDataConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.NetworkElementDataloadDataFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="NetworkElementDataloadDataApplicationResources" key="<%= NetworkElementDataloadDataConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.NetworkElementDataloadDataFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitNetworkElementDataloadDataAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.NetworkElementDataloadDataFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(NetworkElementDataloadDataConstants.USER) == null) {
  response.sendRedirect(NetworkElementDataloadDataConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="NetworkElementDataloadDataApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitNetworkElementDataloadDataAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
