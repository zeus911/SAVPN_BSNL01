...
String tabName = (String) request.getParameter(DL_InterfaceConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.DL_InterfaceFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="DL_InterfaceApplicationResources" key="<%= DL_InterfaceConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.DL_InterfaceFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitDL_InterfaceAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.DL_InterfaceFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(DL_InterfaceConstants.USER) == null) {
  response.sendRedirect(DL_InterfaceConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="DL_InterfaceApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitDL_InterfaceAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
