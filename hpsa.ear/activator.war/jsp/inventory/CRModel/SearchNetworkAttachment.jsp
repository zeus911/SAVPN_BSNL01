...
if(form==null) {
 form=new NetworkAttachmentForm();
} 
    
      
  
String datasource = (String) request.getParameter(NetworkAttachmentConstants.DATASOURCE);
String tabName = (String) request.getParameter(NetworkAttachmentConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.NetworkAttachmentFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="NetworkAttachmentApplicationResources" key="<%= NetworkAttachmentConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.NetworkAttachmentFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitNetworkAttachmentAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.NetworkAttachmentFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(NetworkAttachmentConstants.USER) == null) {
  response.sendRedirect(NetworkAttachmentConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="NetworkAttachmentApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitNetworkAttachmentAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
