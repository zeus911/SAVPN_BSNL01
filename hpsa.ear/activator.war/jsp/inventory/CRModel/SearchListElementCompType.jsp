...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(ElementCompTypeConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(ElementCompTypeConstants.DATASOURCE);
String tabName = request.getParameter(ElementCompTypeConstants.TAB_NAME);
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListElementCompType.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="ElementCompTypeApplicationResources" key="<%= ElementCompTypeConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="ElementCompTypeApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitElementCompTypeAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( Type___hide == null || Type___hide.equals("null") ) {
%>
      <display:column property="type" sortable="true" titleKey="ElementCompTypeApplicationResources:field.type.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( ElementTypeGroupName___hide == null || ElementTypeGroupName___hide.equals("null") ) {
%>
      <display:column property="elementtypegroupname" sortable="true" titleKey="ElementCompTypeApplicationResources:field.elementtypegroupname.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( ECType___hide == null || ECType___hide.equals("null") ) {
%>
      <display:column property="ectype" sortable="true" titleKey="ElementCompTypeApplicationResources:field.ectype.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormElementCompTypeAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.ElementCompTypeForm.submit()"/>
  </html:form>
