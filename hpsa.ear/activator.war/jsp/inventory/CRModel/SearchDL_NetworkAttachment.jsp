...
if(form==null) {
 form=new DL_NetworkAttachmentForm();
} 
    
      
  
String datasource = (String) request.getParameter(DL_NetworkAttachmentConstants.DATASOURCE);
String tabName = (String) request.getParameter(DL_NetworkAttachmentConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.DL_NetworkAttachmentFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="DL_NetworkAttachmentApplicationResources" key="<%= DL_NetworkAttachmentConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.DL_NetworkAttachmentFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitDL_NetworkAttachmentAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.DL_NetworkAttachmentFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(DL_NetworkAttachmentConstants.USER) == null) {
  response.sendRedirect(DL_NetworkAttachmentConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="DL_NetworkAttachmentApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitDL_NetworkAttachmentAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
