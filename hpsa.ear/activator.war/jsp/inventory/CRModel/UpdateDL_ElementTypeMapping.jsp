...
String formAction = "/UpdateCommitDL_ElementTypeMappingAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.DL_ElementTypeMappingForm.action = '/activator<%=moduleConfig%>/UpdateFormDL_ElementTypeMappingAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.DL_ElementTypeMappingForm.submit();
    }
    function performCommit()
    {
      window.document.DL_ElementTypeMappingForm.action = '/activator<%=moduleConfig%>/UpdateCommitDL_ElementTypeMappingAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.DL_ElementTypeMappingForm.submit();
    }
    function init()
    {
...
if(beanDL_ElementTypeMapping==null)
   beanDL_ElementTypeMapping = (com.hp.ov.activator.cr.inventory.DL_ElementTypeMapping) request.getSession().getAttribute(DL_ElementTypeMappingConstants.DL_ELEMENTTYPEMAPPING_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String Id = beanDL_ElementTypeMapping.getId();
        
            
                            
            
                
                String deviceModel = beanDL_ElementTypeMapping.getDevicemodel();
        
            
                            
            
                
                String ElementTypeGroup = beanDL_ElementTypeMapping.getElementtypegroup();
        
            
                            
            
                
                String ElementType = beanDL_ElementTypeMapping.getElementtype();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="DL_ElementTypeMappingApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                                        <html:text  property="devicemodel" size="24" value="<%= deviceModel %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="elementtypegroup" size="24" value="<%= ElementTypeGroup %>"/>
                                                </table:cell>
              <table:cell>
