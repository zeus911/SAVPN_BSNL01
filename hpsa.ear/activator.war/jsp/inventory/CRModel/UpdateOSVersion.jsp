...
String formAction = "/UpdateCommitOSVersionAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("osversionname") ;
}
...
<html>
  <head>
    ...
    {
      window.document.OSVersionForm.action = '/activator<%=moduleConfig%>/UpdateFormOSVersionAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.OSVersionForm.submit();
    }
    function performCommit()
    {
      window.document.OSVersionForm.action = '/activator<%=moduleConfig%>/UpdateCommitOSVersionAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.OSVersionForm.submit();
    }
    function init()
    {
...
if(beanOSVersion==null)
   beanOSVersion = (com.hp.ov.activator.cr.inventory.OSVersion) request.getSession().getAttribute(OSVersionConstants.OSVERSION_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String OSVersionName = beanOSVersion.getOsversionname();
        
            
                            
            
                
                String OSVersionGroup = beanOSVersion.getOsversiongroup();
        
          String OSVersionGroupLabel = (String) request.getAttribute(OSVersionConstants.OSVERSIONGROUP_LABEL);
      ArrayList OSVersionGroupListOfValues = (ArrayList) request.getAttribute(OSVersionConstants.OSVERSIONGROUP_LIST_OF_VALUES);
            
                            
            
                
                String Description = beanOSVersion.getDescription();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="OSVersionApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="osversionname" value="<%= OSVersionName %>"/>
                                                        <html:text disabled="true" property="osversionname" size="24" value="<%= OSVersionName %>"/>
                                                </table:cell>
              <table:cell>
