...
String formAction = "/UpdateCommitDL_NetworkAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.DL_NetworkForm.action = '/activator<%=moduleConfig%>/UpdateFormDL_NetworkAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.DL_NetworkForm.submit();
    }
    function performCommit()
    {
      window.document.DL_NetworkForm.action = '/activator<%=moduleConfig%>/UpdateCommitDL_NetworkAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.DL_NetworkForm.submit();
    }
    function init()
    {
...
if(beanDL_Network==null)
   beanDL_Network = (com.hp.ov.activator.nnmi.dl.inventory.DL_Network) request.getSession().getAttribute(DL_NetworkConstants.DL_NETWORK_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="DL_NetworkApplicationResources" key="jsp.update.title"/>
</h2> 
