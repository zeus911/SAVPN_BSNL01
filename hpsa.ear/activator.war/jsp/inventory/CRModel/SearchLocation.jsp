...
String tabName = (String) request.getParameter(LocationConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.LocationFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="LocationApplicationResources" key="<%= LocationConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.LocationFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitLocationAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.LocationFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(LocationConstants.USER) == null) {
  response.sendRedirect(LocationConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="LocationApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitLocationAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
