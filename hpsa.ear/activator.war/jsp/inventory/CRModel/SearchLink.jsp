...
String tabName = (String) request.getParameter(LinkConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.LinkFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="LinkApplicationResources" key="<%= LinkConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.LinkFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitLinkAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.LinkFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(LinkConstants.USER) == null) {
  response.sendRedirect(LinkConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="LinkApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitLinkAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
