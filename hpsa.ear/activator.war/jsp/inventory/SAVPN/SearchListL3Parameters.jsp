...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(L3ParametersConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(L3ParametersConstants.DATASOURCE);
String tabName = request.getParameter(L3ParametersConstants.TAB_NAME);
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListL3Parameters.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="L3ParametersApplicationResources" key="<%= L3ParametersConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="L3ParametersApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitL3ParametersAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( Name___hide == null || Name___hide.equals("null") ) {
%>
      <display:column property="name" sortable="true" titleKey="L3ParametersApplicationResources:field.name.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Value___hide == null || Value___hide.equals("null") ) {
%>
      <display:column property="value" sortable="true" titleKey="L3ParametersApplicationResources:field.value.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Description___hide == null || Description___hide.equals("null") ) {
%>
      <display:column property="description" sortable="true" titleKey="L3ParametersApplicationResources:field.description.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormL3ParametersAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.L3ParametersForm.submit()"/>
  </html:form>
