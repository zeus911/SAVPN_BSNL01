...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(TrafficClassifierConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(TrafficClassifierConstants.DATASOURCE);
String tabName = request.getParameter(TrafficClassifierConstants.TAB_NAME);
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListTrafficClassifier.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="TrafficClassifierApplicationResources" key="<%= TrafficClassifierConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="TrafficClassifierApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitTrafficClassifierAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( Name___hide == null || Name___hide.equals("null") ) {
%>
      <display:column property="name" sortable="true" titleKey="TrafficClassifierApplicationResources:field.name.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( CustomerId___hide == null || CustomerId___hide.equals("null") ) {
%>
      <display:column property="customerid" sortable="true" titleKey="TrafficClassifierApplicationResources:field.customerid.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Layer___hide == null || Layer___hide.equals("null") ) {
%>
      <display:column property="layer" sortable="true" titleKey="TrafficClassifierApplicationResources:field.layer.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( DSCPs___hide == null || DSCPs___hide.equals("null") ) {
%>
      <display:column property="dscps" sortable="true" titleKey="TrafficClassifierApplicationResources:field.dscps.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Filter___hide == null || Filter___hide.equals("null") ) {
%>
      <display:column property="filter" sortable="true" titleKey="TrafficClassifierApplicationResources:field.filter.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( CoSs___hide == null || CoSs___hide.equals("null") ) {
%>
      <display:column property="coss" sortable="true" titleKey="TrafficClassifierApplicationResources:field.coss.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Compliant___hide == null || Compliant___hide.equals("null") ) {
%>
      <display:column property="compliant" sortable="true" titleKey="TrafficClassifierApplicationResources:field.compliant.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( AddressFamily___hide == null || AddressFamily___hide.equals("null") ) {
%>
      <display:column property="addressfamily" sortable="true" titleKey="TrafficClassifierApplicationResources:field.addressfamily.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormTrafficClassifierAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.TrafficClassifierForm.submit()"/>
  </html:form>
