...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(VRFConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(VRFConstants.DATASOURCE);
String tabName = request.getParameter(VRFConstants.TAB_NAME);
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListVRF.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="VRFApplicationResources" key="<%= VRFConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="VRFApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitVRFAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( VRFName___hide == null || VRFName___hide.equals("null") ) {
%>
      <display:column property="vrfname" sortable="true" titleKey="VRFApplicationResources:field.vrfname.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( NE_ID___hide == null || NE_ID___hide.equals("null") ) {
%>
      <display:column property="ne_id" sortable="true" titleKey="VRFApplicationResources:field.ne_id.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( RD___hide == null || RD___hide.equals("null") ) {
%>
      <display:column property="rd" sortable="true" titleKey="VRFApplicationResources:field.rd.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormVRFAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.VRFForm.submit()"/>
  </html:form>
