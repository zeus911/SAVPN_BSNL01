...
String tabName = (String) request.getParameter(Sh_CustomerConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_CustomerFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_CustomerApplicationResources" key="<%= Sh_CustomerConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_CustomerFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_CustomerAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_CustomerFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_CustomerConstants.USER) == null) {
  response.sendRedirect(Sh_CustomerConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_CustomerApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitSh_CustomerAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
