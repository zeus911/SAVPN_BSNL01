...
String formAction = "/UpdateCommitSh_VPNAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.Sh_VPNForm.action = '/activator<%=moduleConfig%>/UpdateFormSh_VPNAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.Sh_VPNForm.submit();
    }
    function performCommit()
    {
      window.document.Sh_VPNForm.action = '/activator<%=moduleConfig%>/UpdateCommitSh_VPNAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.Sh_VPNForm.submit();
    }
    function init()
    {
...
if(beanSh_VPN==null)
   beanSh_VPN = (com.hp.ov.activator.vpn.inventory.Sh_VPN) request.getSession().getAttribute(Sh_VPNConstants.SH_VPN_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_VPNApplicationResources" key="jsp.update.title"/>
</h2> 
