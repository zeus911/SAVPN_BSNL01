...
String tabName = (String) request.getParameter(L2VCConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.L2VCFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
                    if(document.getElementsByName("linkid___hide")[0].checked) {
                            ...
                            if(document.getElementsByName("l2vpn___hide")[0].checked) {
                            if(document.getElementsByName("bpdutunnel___hide")[0].checked) {
              checkfalse=true;
                    }
                            }
                                      }
            ...
            if(checkfalse){
    alert("<bean:message bundle="L2VCApplicationResources" key="<%= L2VCConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.L2VCFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitL2VCAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.L2VCFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(L2VCConstants.USER) == null) {
  response.sendRedirect(L2VCConstants.NULL_SESSION);
  return;
}
...
<%
      String LinkId = form.getLinkid();
          ...
          String NNMi_LastUpdateData___ = form.getNnmi_lastupdatedata___();
            String L2VPN = form.getL2vpn();
            String BPDUTunnel = form.getBpdutunnel();
      
%>

<h2 style="width:100%; text-align:center;">
  <bean:message bundle="L2VCApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitL2VCAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
      ...
      <table:header>
        ...
        <table:cell>
          <bean:message bundle="InventoryResources" key="description.heading"/>
            ...
            <table:cell>
              <center>
                <html:checkbox property="l2vpn___hide" value="true"/>
              </center>
            </table:cell>
            ...
            <table:cell>
                              <html:text property="l2vpn" size="16" value="<%= L2VPN %>"/>
                                          </table:cell>
            ...
            <table:cell>
              <center>
                <html:checkbox property="bpdutunnel___hide" value="true"/>
              </center>
            </table:cell>
            ...
            <table:cell>
                              <html:text property="bpdutunnel" size="16" value="<%= BPDUTunnel %>"/>
                                          </table:cell>
