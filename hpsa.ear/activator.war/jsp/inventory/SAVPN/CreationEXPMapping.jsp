...
String formAction = "/CreationCommitEXPMappingAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.EXPMappingForm.action = '/activator<%=moduleConfig%>/CreationFormEXPMappingAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_location_=' + focusthis;
      window.document.EXPMappingForm.submit();
    }
    function performCommit()
    {
      window.document.EXPMappingForm.action = '/activator<%=moduleConfig%>/CreationCommitEXPMappingAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.EXPMappingForm.submit();
    }
    function init()
    {
...
com.hp.ov.activator.vpn.inventory.EXPMapping beanEXPMapping = (com.hp.ov.activator.vpn.inventory.EXPMapping) request.getAttribute(EXPMappingConstants.EXPMAPPING_BEAN);
      ...
      String Position = beanEXPMapping.getPosition();
                String ClassName = beanEXPMapping.getClassname();
                String ExpValue = beanEXPMapping.getExpvalue();
                String DSCPValue = beanEXPMapping.getDscpvalue();
                String PLP = beanEXPMapping.getPlp();
                String QueueName = beanEXPMapping.getQueuename();
          
%>

<h2 style="width:100%; text-align:center;">
  <bean:message bundle="EXPMappingApplicationResources" key="jsp.creation.title"/>
</h2> 
...
<html:form action="<%= formAction %>" style="text-align:center;">
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                                        <html:text  property="classname" size="24" value="<%= ClassName %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="expvalue" size="24" value="<%= ExpValue %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="dscpvalue" size="24" value="<%= DSCPValue %>"/>
                                                </table:cell>
              <table:cell>
