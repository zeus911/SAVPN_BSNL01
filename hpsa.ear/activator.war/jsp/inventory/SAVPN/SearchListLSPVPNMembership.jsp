...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(LSPVPNMembershipConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(LSPVPNMembershipConstants.DATASOURCE);
String tabName = request.getParameter(LSPVPNMembershipConstants.TAB_NAME);
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListLSPVPNMembership.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="LSPVPNMembershipApplicationResources" key="<%= LSPVPNMembershipConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="LSPVPNMembershipApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitLSPVPNMembershipAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( VPNId___hide == null || VPNId___hide.equals("null") ) {
%>
      <display:column property="vpnid" sortable="true" titleKey="LSPVPNMembershipApplicationResources:field.vpnid.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( LSPId___hide == null || LSPId___hide.equals("null") ) {
%>
      <display:column property="lspid" sortable="true" titleKey="LSPVPNMembershipApplicationResources:field.lspid.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormLSPVPNMembershipAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.LSPVPNMembershipForm.submit()"/>
  </html:form>
