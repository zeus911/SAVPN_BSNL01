...
String tabName = (String) request.getParameter(FlowPointConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.FlowPointFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="FlowPointApplicationResources" key="<%= FlowPointConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.FlowPointFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitFlowPointAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.FlowPointFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(FlowPointConstants.USER) == null) {
  response.sendRedirect(FlowPointConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="FlowPointApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitFlowPointAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
