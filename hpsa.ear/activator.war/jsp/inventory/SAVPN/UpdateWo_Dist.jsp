...
String formAction = "/UpdateCommitWo_DistAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("manual") ;
}
...
<html>
  <head>
    ...
    {
      window.document.Wo_DistForm.action = '/activator<%=moduleConfig%>/UpdateFormWo_DistAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.Wo_DistForm.submit();
    }
    function performCommit()
    {
      window.document.Wo_DistForm.action = '/activator<%=moduleConfig%>/UpdateCommitWo_DistAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.Wo_DistForm.submit();
    }
    function init()
    {
...
if(beanWo_Dist==null)
   beanWo_Dist = (com.hp.ov.activator.vpn.inventory.Wo_Dist) request.getSession().getAttribute(Wo_DistConstants.WO_DIST_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String Manual = beanWo_Dist.getManual();
        
            
                            
            
                
                String Email = beanWo_Dist.getEmail();
        
            
                            
            
                
                String WebServer = beanWo_Dist.getWebserver();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Wo_DistApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="manual" value="<%= Manual %>"/>
                                                        <html:text disabled="true" property="manual" size="24" value="<%= Manual %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                        ...
                        selValue=Email.toString();    
                          %>

                    <html:select  property="email" value="<%= selValue %>" >
              ...
              <table:cell>
                                                                          <%
                        String selValue=null;                                    
                        if(WebServer==null||WebServer.trim().equals(""))
                           selValue="false";
                        else
                        selValue=WebServer.toString();    
                          %>

                    <html:select  property="webserver" value="<%= selValue %>" >
                                            <html:option value="true" >true</html:option>
                                            <html:option value="false" >false</html:option>
                                          </html:select>
                                                </table:cell>
