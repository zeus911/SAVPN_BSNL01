...
if(form==null) {
 form=new SetupCE_WorkorderForm();
} 
...
String tabName = (String) request.getParameter(SetupCE_WorkorderConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.SetupCE_WorkorderFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
                    if(document.getElementsByName("serviceid___hide")[0].checked) {
                            if(document.getElementsByName("woname___hide")[0].checked) {
            ...
            if(checkfalse){
    alert("<bean:message bundle="SetupCE_WorkorderApplicationResources" key="<%= SetupCE_WorkorderConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.SetupCE_WorkorderFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSetupCE_WorkorderAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.SetupCE_WorkorderFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(SetupCE_WorkorderConstants.USER) == null) {
  response.sendRedirect(SetupCE_WorkorderConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="SetupCE_WorkorderApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitSetupCE_WorkorderAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
      ...
      <table:header>
        ...
        <table:cell>
          <bean:message bundle="InventoryResources" key="description.heading"/>
            ...
            <table:cell>
              <center>
                <html:checkbox property="woname___hide" value="true"/>
              </center>
            </table:cell>
            ...
            <table:cell>
                              <html:text property="woname" size="16" value="<%= WOName %>"/>
                                          </table:cell>
