...
String formAction = "/UpdateCommitSh_RCAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.Sh_RCForm.action = '/activator<%=moduleConfig%>/UpdateFormSh_RCAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.Sh_RCForm.submit();
    }
    function performCommit()
    {
      window.document.Sh_RCForm.action = '/activator<%=moduleConfig%>/UpdateCommitSh_RCAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.Sh_RCForm.submit();
    }
    function init()
    {
...
if(beanSh_RC==null)
   beanSh_RC = (com.hp.ov.activator.vpn.inventory.Sh_RC) request.getSession().getAttribute(Sh_RCConstants.SH_RC_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_RCApplicationResources" key="jsp.update.title"/>
</h2> 
