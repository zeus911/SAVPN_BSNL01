...
if(form==null) {
 form=new Sh_TerminationPointForm();
} 
...
String tabName = (String) request.getParameter(Sh_TerminationPointConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_TerminationPointFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
                    if(document.getElementsByName("terminationpointid___hide")[0].checked) {
                      ...
                      if(checkfalse){
    alert("<bean:message bundle="Sh_TerminationPointApplicationResources" key="<%= Sh_TerminationPointConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_TerminationPointFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_TerminationPointAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_TerminationPointFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_TerminationPointConstants.USER) == null) {
  response.sendRedirect(Sh_TerminationPointConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_TerminationPointApplicationResources" key="jsp.search.title"/>
</h2> 
...
<div style="color:#ff0000;">
      <html:errors bundle="Sh_TerminationPointApplicationResources" property="TerminationPointID"/>
        ...
        <html:errors bundle="Sh_TerminationPointApplicationResources" property="__count"/>
  <html:errors bundle="InventoryResources" property="warning"/>
</div>
<% String searchFormAction = "/SearchCommitSh_TerminationPointAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
