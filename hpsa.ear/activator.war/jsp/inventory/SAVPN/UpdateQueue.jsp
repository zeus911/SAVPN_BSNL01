...
String formAction = "/UpdateCommitQueueAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.QueueForm.action = '/activator<%=moduleConfig%>/UpdateFormQueueAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.QueueForm.submit();
    }
    function performCommit()
    {
      window.document.QueueForm.action = '/activator<%=moduleConfig%>/UpdateCommitQueueAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.QueueForm.submit();
    }
    function init()
    {
...
if(beanQueue==null)
   beanQueue = (com.hp.ov.activator.vpn.inventory.Queue) request.getSession().getAttribute(QueueConstants.QUEUE_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String Name = beanQueue.getName();
        
            
                            
            
                
                String Type = beanQueue.getType();
        
            
                            
            
                
                String State = beanQueue.getState();
        
            
                            
            
                
                String EmailServer = beanQueue.getEmailserver();
        
            
                            
            
                
                String NB_Service = beanQueue.getNb_service();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="QueueApplicationResources" key="jsp.update.title"/>
</h2> 
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                    ...
                    <html:select disabled="true" property="type" value="<%= selValue %>" >
                                            <html:option value="Workorder" >Workorder</html:option>
                                            <html:option value="NA" >NA</html:option>
                                            <html:option value="NNM" >NNM</html:option>
                                          </html:select>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                    ...
                    <html:select disabled="true" property="state" value="<%= selValue %>" >
                                            <html:option value="active" >active</html:option>
                                            <html:option value="disabled" >disabled</html:option>
                                            <html:option value="suspended" >suspended</html:option>
                                          </html:select>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="emailserver" size="24" value="<%= EmailServer %>"/>
                                                </table:cell>
              <table:cell>
                <bean:message bundle="QueueApplicationResources" key="field.emailserver.description"/>
                                                    ...
                                                    <html:hidden property="nb_service" value="<%= NB_Service %>"/>            
              ...
              <table:cell>
                                                                        <html:text  property="sleeptime" size="24" value="<%= SleepTime %>"/>
                                                </table:cell>
              <table:cell>
