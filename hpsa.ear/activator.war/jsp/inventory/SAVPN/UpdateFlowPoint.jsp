...
String formAction = "/UpdateCommitFlowPointAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.FlowPointForm.action = '/activator<%=moduleConfig%>/UpdateFormFlowPointAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.FlowPointForm.submit();
    }
    function performCommit()
    {
      window.document.FlowPointForm.action = '/activator<%=moduleConfig%>/UpdateCommitFlowPointAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.FlowPointForm.submit();
    }
    function init()
    {
...
if(beanFlowPoint==null)
   beanFlowPoint = (com.hp.ov.activator.vpn.inventory.FlowPoint) request.getSession().getAttribute(FlowPointConstants.FLOWPOINT_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="FlowPointApplicationResources" key="jsp.update.title"/>
</h2> 
