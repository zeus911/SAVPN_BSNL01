...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(VlanRangeConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(VlanRangeConstants.DATASOURCE);
String tabName = request.getParameter(VlanRangeConstants.TAB_NAME);
...
VlanRangeForm form = (VlanRangeForm) request.getAttribute("VlanRangeForm");


String VlanRangeID___hide = null;
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListVlanRange.jsp?");
...
if ( form != null ) {
  VlanRangeID___hide = form.getVlanrangeid___hide();
  ...
  if ( VlanRangeID___hide != null )
    requestURI.append("vlanrangeid___hide=" + VlanRangeID___hide);
  ...
  if ( Region___hide != null )
    ...
    VlanRangeID___hide = request.getParameter("vlanrangeid___hide");
...
<html>
  <head>
    <title><bean:message bundle="VlanRangeApplicationResources" key="<%= VlanRangeConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="VlanRangeApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitVlanRangeAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( VlanRangeID___hide == null || VlanRangeID___hide.equals("null") ) {
%>
      <display:column property="vlanrangeid" sortable="true" titleKey="VlanRangeApplicationResources:field.vlanrangeid.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Usage___hide == null || Usage___hide.equals("null") ) {
%>
      <display:column property="usage" sortable="true" titleKey="VlanRangeApplicationResources:field.usage.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Allocation___hide == null || Allocation___hide.equals("null") ) {
%>
      <display:column property="allocation" sortable="true" titleKey="VlanRangeApplicationResources:field.allocation.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( StartValue___hide == null || StartValue___hide.equals("null") ) {
%>
      <display:column property="startvalue" sortable="true" titleKey="VlanRangeApplicationResources:field.startvalue.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( EndValue___hide == null || EndValue___hide.equals("null") ) {
%>
      <display:column property="endvalue" sortable="true" titleKey="VlanRangeApplicationResources:field.endvalue.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Description___hide == null || Description___hide.equals("null") ) {
%>
      <display:column property="description" sortable="true" titleKey="VlanRangeApplicationResources:field.description.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Region___hide == null || Region___hide.equals("null") ) {
%>
      <display:column property="region" sortable="true" titleKey="VlanRangeApplicationResources:field.region.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormVlanRangeAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
  <html:form action="<%= backAction %>">
<%
  if ( form != null ) {
%>
                  <html:hidden property="vlanrangeid" value="<%= form.getVlanrangeid() %>"/>
                <html:hidden property="vlanrangeid___hide" value="<%= form.getVlanrangeid___hide() %>"/>
...
%>
                  <html:hidden property="vlanrangeid" value="<%= request.getParameter(\"vlanrangeid\") %>"/>
                <html:hidden property="vlanrangeid___hide" value="<%= request.getParameter(\"vlanrangeid___hide\") %>"/>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.VlanRangeForm.submit()"/>
  </html:form>
