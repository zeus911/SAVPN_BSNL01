...
String formAction = "/UpdateCommitAdminVPNAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.AdminVPNForm.action = '/activator<%=moduleConfig%>/UpdateFormAdminVPNAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.AdminVPNForm.submit();
    }
    function performCommit()
    {
      window.document.AdminVPNForm.action = '/activator<%=moduleConfig%>/UpdateCommitAdminVPNAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.AdminVPNForm.submit();
    }
    function init()
    {
...
if(beanAdminVPN==null)
   beanAdminVPN = (com.hp.ov.activator.vpn.inventory.AdminVPN) request.getSession().getAttribute(AdminVPNConstants.ADMINVPN_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String VRFID = beanAdminVPN.getVrfid();
        
            
                            
            
                
                String AdminVRFName = beanAdminVPN.getAdminvrfname();
        
            
                            
            
                
                String RD = beanAdminVPN.getRd();
        
            
                            
            
                
                String RTExport = beanAdminVPN.getRtexport();
        
            
                            
            
                
                String RTImport = beanAdminVPN.getRtimport();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="AdminVPNApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="vrfid" value="<%= VRFID %>"/>
                                                        <html:text disabled="true" property="vrfid" size="24" value="<%= VRFID %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="adminvrfname" size="24" value="<%= AdminVRFName %>"/>
                                                </table:cell>
              <table:cell>
