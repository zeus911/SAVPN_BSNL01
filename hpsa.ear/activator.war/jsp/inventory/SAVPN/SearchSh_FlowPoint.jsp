...
String tabName = (String) request.getParameter(Sh_FlowPointConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_FlowPointFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_FlowPointApplicationResources" key="<%= Sh_FlowPointConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_FlowPointFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_FlowPointAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_FlowPointFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_FlowPointConstants.USER) == null) {
  response.sendRedirect(Sh_FlowPointConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_FlowPointApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitSh_FlowPointAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
