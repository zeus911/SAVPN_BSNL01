...
String tabName = (String) request.getParameter(Sh_VPNConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_VPNFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_VPNApplicationResources" key="<%= Sh_VPNConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_VPNFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_VPNAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_VPNFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_VPNConstants.USER) == null) {
  response.sendRedirect(Sh_VPNConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_VPNApplicationResources" key="jsp.search.title"/>
</h2> 
...
<div style="color:#ff0000;">
      <html:errors bundle="Sh_VPNApplicationResources" property="ServiceId"/>
        ...
        <html:errors bundle="Sh_VPNApplicationResources" property="__count"/>
...
<% String searchFormAction = "/SearchCommitSh_VPNAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
