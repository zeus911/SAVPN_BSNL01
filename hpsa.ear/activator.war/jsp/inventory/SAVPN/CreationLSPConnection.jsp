...
String formAction = "/CreationCommitLSPConnectionAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.LSPConnectionForm.action = '/activator<%=moduleConfig%>/CreationFormLSPConnectionAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_location_=' + focusthis;
      window.document.LSPConnectionForm.submit();
    }
    function performCommit()
    {
      window.document.LSPConnectionForm.action = '/activator<%=moduleConfig%>/CreationCommitLSPConnectionAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.LSPConnectionForm.submit();
    }
    function init()
    {
...
com.hp.ov.activator.vpn.inventory.LSPConnection beanLSPConnection = (com.hp.ov.activator.vpn.inventory.LSPConnection) request.getAttribute(LSPConnectionConstants.LSPCONNECTION_BEAN);
      ...
      String headPE = beanLSPConnection.getHeadpe();
                String tailPE = beanLSPConnection.getTailpe();
                String UsageMode = beanLSPConnection.getUsagemode();
                String VpnId = beanLSPConnection.getVpnid();
          
%>

<h2 style="width:100%; text-align:center;">
  <bean:message bundle="LSPConnectionApplicationResources" key="jsp.creation.title"/>
</h2> 
...
<html:form action="<%= formAction %>" style="text-align:center;">
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                                        <html:text  property="headpe" size="24" value="<%= headPE %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                                        <html:text  property="tailpe" size="24" value="<%= tailPE %>"/>
                                                </table:cell>
              <table:cell>
