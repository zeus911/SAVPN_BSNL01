...
String formAction = "/UpdateCommitActivePoliciesAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.ActivePoliciesForm.action = '/activator<%=moduleConfig%>/UpdateFormActivePoliciesAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.ActivePoliciesForm.submit();
    }
    function performCommit()
    {
      window.document.ActivePoliciesForm.action = '/activator<%=moduleConfig%>/UpdateCommitActivePoliciesAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.ActivePoliciesForm.submit();
    }
    function init()
    {
...
if(beanActivePolicies==null)
   beanActivePolicies = (com.hp.ov.activator.vpn.inventory.ActivePolicies) request.getSession().getAttribute(ActivePoliciesConstants.ACTIVEPOLICIES_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String ID = beanActivePolicies.getId();
        
            
                            
            
                
                String SchedulingPolicyName = beanActivePolicies.getSchedulingpolicyname();
        
            
                            
            
                
                String RetrievalPolicyName = beanActivePolicies.getRetrievalpolicyname();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="ActivePoliciesApplicationResources" key="jsp.update.title"/>
</h2> 
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="id" value="<%= ID %>"/>
                                                        <html:text disabled="true" property="id" size="24" value="<%= ID %>"/>
                                                </table:cell>
              <table:cell>
