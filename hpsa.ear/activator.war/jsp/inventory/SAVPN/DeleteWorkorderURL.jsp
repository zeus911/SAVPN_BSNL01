...
<html>
  <head>
    <title><bean:message bundle="WorkorderURLApplicationResources" key="<%= WorkorderURLConstants.JSP_DELETE_TITLE %>"/></title>
    ...
    {
      window.document.WorkorderURLForm.action = '/activator<%=moduleConfig%>/DeleteCommitWorkorderURLAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.WorkorderURLForm.submit();
    }
    function init()
    {
      var confirm = new HPSAConfirm('Confirm','<bean:message bundle="WorkorderURLApplicationResources" key="delete.confirm"/>', '<bean:message bundle="InventoryResources" key="confirm.ok_button.label"/>', '<bean:message bundle="InventoryResources" key="confirm.cancel_button.label"/>');
...
nfB.setMaximumFractionDigits(6);
         ...
         WODistributionID = "";
                      ...
                      String Webserver = StringFacility.replaceAllByHTMLCharacter(beanWorkorderURL.getWebserver());
                        
                                  
                      String Email = StringFacility.replaceAllByHTMLCharacter(beanWorkorderURL.getEmail());
                        
                                  
                      String Location = StringFacility.replaceAllByHTMLCharacter(beanWorkorderURL.getLocation());
                        
                                  
                      String Region = StringFacility.replaceAllByHTMLCharacter(beanWorkorderURL.getRegion());
                        
                                  
    
%>
<h2 style="width:100%; text-align:center;">
    <bean:message bundle="WorkorderURLApplicationResources" key="jsp.delete.title"/>
</h2> 
...
%>
      ...
      <table:header>
        ...
        <table:cell>
          <bean:message bundle="InventoryResources" key="description.heading"/>
            ...
            <table:cell>
                            <%= WODistributionID != null? WODistributionID : "" %>
                          </table:cell>
            <table:cell>
    ...
    <html:form action="/DeleteCommitWorkorderURLAction.do">
    <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
                  <html:hidden property="wodistributionid" value="<%= String.valueOf(WODistributionID) %>"/>
              </html:form>
  </body>
</html>
