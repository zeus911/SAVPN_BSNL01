...
String formAction = "/UpdateCommitSchedulingPolicyAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("schedulingpolicyname") ;
}
...
<html>
  <head>
    ...
    {
      window.document.SchedulingPolicyForm.action = '/activator<%=moduleConfig%>/UpdateFormSchedulingPolicyAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.SchedulingPolicyForm.submit();
    }
    function performCommit()
    {
      window.document.SchedulingPolicyForm.action = '/activator<%=moduleConfig%>/UpdateCommitSchedulingPolicyAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.SchedulingPolicyForm.submit();
    }
    function init()
    {
...
if(beanSchedulingPolicy==null)
   beanSchedulingPolicy = (com.hp.ov.activator.vpn.inventory.SchedulingPolicy) request.getSession().getAttribute(SchedulingPolicyConstants.SCHEDULINGPOLICY_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="SchedulingPolicyApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="schedulingpolicyname" value="<%= SchedulingPolicyName %>"/>
                                                        <html:text disabled="true" property="schedulingpolicyname" size="24" value="<%= SchedulingPolicyName %>"/>
                                                </table:cell>
              <table:cell>
