...
String tabName = (String) request.getParameter(Sh_RCMembershipConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_RCMembershipFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_RCMembershipApplicationResources" key="<%= Sh_RCMembershipConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_RCMembershipFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_RCMembershipAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_RCMembershipFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_RCMembershipConstants.USER) == null) {
  response.sendRedirect(Sh_RCMembershipConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_RCMembershipApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitSh_RCMembershipAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
