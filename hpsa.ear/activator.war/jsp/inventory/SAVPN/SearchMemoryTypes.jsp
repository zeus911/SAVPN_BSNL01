...
String tabName = (String) request.getParameter(MemoryTypesConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.MemoryTypesFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
                    if(document.getElementsByName("memorytype___hide")[0].checked) {
                            if(document.getElementsByName("targettype___hide")[0].checked) {
                            if(document.getElementsByName("os___hide")[0].checked) {
            ...
            if(checkfalse){
    alert("<bean:message bundle="MemoryTypesApplicationResources" key="<%= MemoryTypesConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.MemoryTypesFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitMemoryTypesAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.MemoryTypesFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(MemoryTypesConstants.USER) == null) {
  response.sendRedirect(MemoryTypesConstants.NULL_SESSION);
  return;
}
...
<%
      String MemoryType = form.getMemorytype();
            String TargetType = form.getTargettype();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="MemoryTypesApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitMemoryTypesAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
      ...
      <table:header>
        ...
        <table:cell>
          <bean:message bundle="InventoryResources" key="description.heading"/>
            ...
            <table:cell>
              <center>
                <html:checkbox property="targettype___hide" value="true"/>
              </center>
            </table:cell>
            ...
            <table:cell>
                              <html:text property="targettype" size="16" value="<%= TargetType %>"/>
                                          </table:cell>
            ...
            <table:cell>
              <center>
                <html:checkbox property="os___hide" value="true"/>
              </center>
            </table:cell>
            ...
            <table:cell>
                              <html:text property="os" size="16" value="<%= OS %>"/>
                                          </table:cell>
