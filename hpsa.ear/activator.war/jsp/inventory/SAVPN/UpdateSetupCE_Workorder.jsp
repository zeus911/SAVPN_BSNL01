...
String formAction = "/UpdateCommitSetupCE_WorkorderAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.SetupCE_WorkorderForm.action = '/activator<%=moduleConfig%>/UpdateFormSetupCE_WorkorderAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.SetupCE_WorkorderForm.submit();
    }
    function performCommit()
    {
      window.document.SetupCE_WorkorderForm.action = '/activator<%=moduleConfig%>/UpdateCommitSetupCE_WorkorderAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.SetupCE_WorkorderForm.submit();
    }
    function init()
    {
...
if(beanSetupCE_Workorder==null)
   beanSetupCE_Workorder = (com.hp.ov.activator.vpn.inventory.SetupCE_Workorder) request.getSession().getAttribute(SetupCE_WorkorderConstants.SETUPCE_WORKORDER_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="SetupCE_WorkorderApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                                        <html:textarea  property="wo_content" cols="60" rows="10"
                          value="<%= (WO_Content == null ? \"\" : WO_Content ) %>" style="resize:true;"/>                  
                                                </table:cell>
              <table:cell>
