...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(FlowPointConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(FlowPointConstants.DATASOURCE);
String tabName = request.getParameter(FlowPointConstants.TAB_NAME);
...
FlowPointForm form = (FlowPointForm) request.getAttribute("FlowPointForm");
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListFlowPoint.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="FlowPointApplicationResources" key="<%= FlowPointConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="FlowPointApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitFlowPointAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( TerminationPointId___hide == null || TerminationPointId___hide.equals("null") ) {
%>
      <display:column property="terminationpointid" sortable="true" titleKey="FlowPointApplicationResources:field.terminationpointid.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( AttachmentId___hide == null || AttachmentId___hide.equals("null") ) {
%>
      <display:column property="attachmentid" sortable="true" titleKey="FlowPointApplicationResources:field.attachmentid.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( QoSProfile_in___hide == null || QoSProfile_in___hide.equals("null") ) {
%>
      <display:column property="qosprofile_in" sortable="true" titleKey="FlowPointApplicationResources:field.qosprofile_in.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( QoSProfile_out___hide == null || QoSProfile_out___hide.equals("null") ) {
%>
      <display:column property="qosprofile_out" sortable="true" titleKey="FlowPointApplicationResources:field.qosprofile_out.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( QoSChildEnabled___hide == null || QoSChildEnabled___hide.equals("null") ) {
%>
      <display:column property="qoschildenabled" sortable="true" titleKey="FlowPointApplicationResources:field.qoschildenabled.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( RateLimit_in___hide == null || RateLimit_in___hide.equals("null") ) {
%>
      <display:column property="ratelimit_in" sortable="true" titleKey="FlowPointApplicationResources:field.ratelimit_in.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( RateLimit_out___hide == null || RateLimit_out___hide.equals("null") ) {
%>
      <display:column property="ratelimit_out" sortable="true" titleKey="FlowPointApplicationResources:field.ratelimit_out.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( UsageMode___hide == null || UsageMode___hide.equals("null") ) {
%>
      <display:column property="usagemode" sortable="true" titleKey="FlowPointApplicationResources:field.usagemode.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormFlowPointAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.FlowPointForm.submit()"/>
  </html:form>
