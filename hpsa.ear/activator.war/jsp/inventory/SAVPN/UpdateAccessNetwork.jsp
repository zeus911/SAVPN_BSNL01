...
String formAction = "/UpdateCommitAccessNetworkAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.AccessNetworkForm.action = '/activator<%=moduleConfig%>/UpdateFormAccessNetworkAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.AccessNetworkForm.submit();
    }
    function performCommit()
    {
      window.document.AccessNetworkForm.action = '/activator<%=moduleConfig%>/UpdateCommitAccessNetworkAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.AccessNetworkForm.submit();
    }
    function init()
    {
...
if(beanAccessNetwork==null)
   beanAccessNetwork = (com.hp.ov.activator.vpn.inventory.AccessNetwork) request.getSession().getAttribute(AccessNetworkConstants.ACCESSNETWORK_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="AccessNetworkApplicationResources" key="jsp.update.title"/>
</h2> 
