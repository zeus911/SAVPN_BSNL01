...
String tabName = (String) request.getParameter(Sh_SiteConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_SiteFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_SiteApplicationResources" key="<%= Sh_SiteConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_SiteFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_SiteAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_SiteFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_SiteConstants.USER) == null) {
  response.sendRedirect(Sh_SiteConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_SiteApplicationResources" key="jsp.search.title"/>
</h2> 
...
<div style="color:#ff0000;">
      <html:errors bundle="Sh_SiteApplicationResources" property="ServiceId"/>
        ...
        <html:errors bundle="Sh_SiteApplicationResources" property="__count"/>
...
<% String searchFormAction = "/SearchCommitSh_SiteAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
