...
String tabName = (String) request.getParameter(RCConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.RCFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="RCApplicationResources" key="<%= RCConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.RCFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitRCAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.RCFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(RCConstants.USER) == null) {
  response.sendRedirect(RCConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="RCApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitRCAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
