...
String tabName = (String) request.getParameter(Sh_AccessFlowConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_AccessFlowFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_AccessFlowApplicationResources" key="<%= Sh_AccessFlowConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_AccessFlowFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_AccessFlowAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_AccessFlowFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_AccessFlowConstants.USER) == null) {
  response.sendRedirect(Sh_AccessFlowConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_AccessFlowApplicationResources" key="jsp.search.title"/>
</h2> 
...
<div style="color:#ff0000;">
      <html:errors bundle="Sh_AccessFlowApplicationResources" property="ServiceId"/>
        ...
        <html:errors bundle="Sh_AccessFlowApplicationResources" property="__count"/>
...
<% String searchFormAction = "/SearchCommitSh_AccessFlowAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
