...
String formAction = "/UpdateCommitServiceMappingAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("actionname") + "||" + request.getParameter("service_name") ;
}
...
<html>
  <head>
    ...
    {
      window.document.ServiceMappingForm.action = '/activator<%=moduleConfig%>/UpdateFormServiceMappingAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.ServiceMappingForm.submit();
    }
    function performCommit()
    {
      window.document.ServiceMappingForm.action = '/activator<%=moduleConfig%>/UpdateCommitServiceMappingAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.ServiceMappingForm.submit();
    }
    function init()
    {
...
if(beanServiceMapping==null)
   beanServiceMapping = (com.hp.ov.activator.vpn.inventory.ServiceMapping) request.getSession().getAttribute(ServiceMappingConstants.SERVICEMAPPING_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="ServiceMappingApplicationResources" key="jsp.update.title"/>
</h2> 
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="actionname" value="<%= actionName %>"/>
                                                        <html:text disabled="true" property="actionname" size="24" value="<%= actionName %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                      <html:hidden property="service_name" value="<%= service_name %>"/>
                                                        <html:text disabled="true" property="service_name" size="24" value="<%= service_name %>"/>
                                                </table:cell>
              <table:cell>
