...
if(form==null) {
 form=new Sh_VPNFPMembershipForm();
} 
...
String tabName = (String) request.getParameter(Sh_VPNFPMembershipConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_VPNFPMembershipFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_VPNFPMembershipApplicationResources" key="<%= Sh_VPNFPMembershipConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_VPNFPMembershipFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_VPNFPMembershipAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_VPNFPMembershipFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_VPNFPMembershipConstants.USER) == null) {
  response.sendRedirect(Sh_VPNFPMembershipConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_VPNFPMembershipApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitSh_VPNFPMembershipAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
