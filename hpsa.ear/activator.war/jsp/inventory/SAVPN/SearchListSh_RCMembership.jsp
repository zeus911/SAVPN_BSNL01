...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(Sh_RCMembershipConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(Sh_RCMembershipConstants.DATASOURCE);
String tabName = request.getParameter(Sh_RCMembershipConstants.TAB_NAME);
...
Sh_RCMembershipForm form = (Sh_RCMembershipForm) request.getAttribute("Sh_RCMembershipForm");
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListSh_RCMembership.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="Sh_RCMembershipApplicationResources" key="<%= Sh_RCMembershipConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="Sh_RCMembershipApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitSh_RCMembershipAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( RCName___hide == null || RCName___hide.equals("null") ) {
%>
      <display:column property="rcname" sortable="true" titleKey="Sh_RCMembershipApplicationResources:field.rcname.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( NE_ID___hide == null || NE_ID___hide.equals("null") ) {
%>
      <display:column property="ne_id" sortable="true" titleKey="Sh_RCMembershipApplicationResources:field.ne_id.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( VRFName___hide == null || VRFName___hide.equals("null") ) {
%>
      <display:column property="vrfname" sortable="true" titleKey="Sh_RCMembershipApplicationResources:field.vrfname.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Marker___hide == null || Marker___hide.equals("null") ) {
%>
      <display:column property="marker" sortable="true" titleKey="Sh_RCMembershipApplicationResources:field.marker.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( UploadStatus___hide == null || UploadStatus___hide.equals("null") ) {
%>
      <display:column property="uploadstatus" sortable="true" titleKey="Sh_RCMembershipApplicationResources:field.uploadstatus.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( DBPrimaryKey___hide == null || DBPrimaryKey___hide.equals("null") ) {
%>
      <display:column property="dbprimarykey" sortable="true" titleKey="Sh_RCMembershipApplicationResources:field.dbprimarykey.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormSh_RCMembershipAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.Sh_RCMembershipForm.submit()"/>
  </html:form>
