...
String formAction = "/UpdateCommitL2FlowPointAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.L2FlowPointForm.action = '/activator<%=moduleConfig%>/UpdateFormL2FlowPointAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.L2FlowPointForm.submit();
    }
    function performCommit()
    {
      window.document.L2FlowPointForm.action = '/activator<%=moduleConfig%>/UpdateCommitL2FlowPointAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.L2FlowPointForm.submit();
    }
    function init()
    {
...
<%
com.hp.ov.activator.vpn.inventory.L2FlowPoint beanL2FlowPoint = (com.hp.ov.activator.vpn.inventory.L2FlowPoint) request.getAttribute(L2FlowPointConstants.L2FLOWPOINT_BEAN);
if(beanL2FlowPoint==null)
   beanL2FlowPoint = (com.hp.ov.activator.vpn.inventory.L2FlowPoint) request.getSession().getAttribute(L2FlowPointConstants.L2FLOWPOINT_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String TerminationPointId = beanL2FlowPoint.getTerminationpointid();
        
            
                            
            
                
                String AttachmentId = beanL2FlowPoint.getAttachmentid();
        
            
                            
            
                
                String QoSProfile_in = beanL2FlowPoint.getQosprofile_in();
        
            
                            
            
                
                String QoSProfile_out = beanL2FlowPoint.getQosprofile_out();
        
            
                            
            
                
                String RateLimit_in = beanL2FlowPoint.getRatelimit_in();
        
            
                            
            
                
                String RateLimit_out = beanL2FlowPoint.getRatelimit_out();
        
            
                            
            
                
                String UsageMode = beanL2FlowPoint.getUsagemode();
        
            
                            
            
                
                String ConnectionId = beanL2FlowPoint.getConnectionid();
        
            
                            
            
                
                String ServiceType = beanL2FlowPoint.getServicetype();
        
            
                            
            
                
                String Facing = beanL2FlowPoint.getFacing();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="L2FlowPointApplicationResources" key="jsp.update.title"/>
</h2> 
...
<h1>
      <html:errors bundle="L2FlowPointApplicationResources" property="TerminationPointId"/>
        ...
        <html:errors bundle="L2FlowPointApplicationResources" property="ServiceType"/>
...
<script>
    ...
    <script>
    allEvents = allEvents + "checkShowRulesFacing();";//default invoked when loading HTML
    function checkShowRulesFacing(){
          var ServiceTypePass = false ;
      if (/^L2VPWS$/.test(document.getElementsByName("servicetype")[0].value)) {ServiceTypePass = true;}
            
    var controlTr = document.getElementsByName("facing")[0];
          if (true && ServiceTypePass ){
      ...
      allEvents = allEvents + "addListener(document.getElementsByName('servicetype')[0],'keyup',checkShowRulesFacing);";
   </script>   
    <script>
function doOnLoad()
{
  // hide field
                    document.getElementsByName("terminationpointid")[0].parentNode.parentNode.style.display="none";
                              ...
                              document.getElementsByName("ratelimit_out")[0].parentNode.parentNode.style.display="none";
                                                document.getElementsByName("facing")[0].parentNode.parentNode.style.display="none";
              // check hiding rules
  eval(allEvents);
}
    ...
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                                          <%
                        String selValue=null;                                    
                        if(Facing==null||Facing.trim().equals(""))
                           selValue="aEnd";
                        else
                        selValue=Facing.toString();    
                          %>

                    <html:select  property="facing" value="<%= selValue %>" >
                                            <html:option value="aEnd" >aEnd</html:option>
                                            <html:option value="zEnd" >zEnd</html:option>
                                          </html:select>
                                                </table:cell>
              <table:cell>
