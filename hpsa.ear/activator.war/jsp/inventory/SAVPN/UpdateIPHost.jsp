...
String formAction = "/UpdateCommitIPHostAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.IPHostForm.action = '/activator<%=moduleConfig%>/UpdateFormIPHostAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.IPHostForm.submit();
    }
    function performCommit()
    {
      window.document.IPHostForm.action = '/activator<%=moduleConfig%>/UpdateCommitIPHostAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.IPHostForm.submit();
    }
    function init()
    {
...
if(beanIPHost==null)
   beanIPHost = (com.hp.ov.activator.vpn.inventory.IPHost) request.getSession().getAttribute(IPHostConstants.IPHOST_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String IP = beanIPHost.getIp();
        
            
                            
            
                
                String PoolName = beanIPHost.getPoolname();
        
            
                            
            
                
                String IPStr = beanIPHost.getIpstr();
        
            
                            
            
                
                String ParentIPNetAddr = beanIPHost.getParentipnetaddr();
        
            
                            
            
                
                String StartAddress = beanIPHost.getStartaddress();
        
            
                            
            
                
                String NumberOfEntries = beanIPHost.getNumberofentries();
        
            
                            
            
                
                String AddressFamily = beanIPHost.getAddressfamily();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="IPHostApplicationResources" key="jsp.update.title"/>
</h2> 
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="ip" value="<%= IP %>"/>
                                                        <html:text disabled="true" property="ip" size="24" value="<%= IP %>"/>
                                                </table:cell>
              <table:cell>
