...
String formAction = "/UpdateCommitSh_VRFAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("vrfname") + "||" + request.getParameter("ne_id") ;
}
...
<html>
  <head>
    ...
    {
      window.document.Sh_VRFForm.action = '/activator<%=moduleConfig%>/UpdateFormSh_VRFAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.Sh_VRFForm.submit();
    }
    function performCommit()
    {
      window.document.Sh_VRFForm.action = '/activator<%=moduleConfig%>/UpdateCommitSh_VRFAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.Sh_VRFForm.submit();
    }
    function init()
    {
...
if(beanSh_VRF==null)
   beanSh_VRF = (com.hp.ov.activator.vpn.inventory.Sh_VRF) request.getSession().getAttribute(Sh_VRFConstants.SH_VRF_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_VRFApplicationResources" key="jsp.update.title"/>
</h2> 
