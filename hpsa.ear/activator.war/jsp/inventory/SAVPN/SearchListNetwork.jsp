...
ArrayList al = (ArrayList) session.getAttribute(request.getParameter(NetworkConstants.TAB_NAME));
String useRandomColor="1";
String mainColor ="1";

String datasource = request.getParameter(NetworkConstants.DATASOURCE);
String tabName = request.getParameter(NetworkConstants.TAB_NAME);
...
StringBuffer requestURI = new StringBuffer("/activator/jsp/inventory/SearchListNetwork.jsp?");
...
<html>
  <head>
    <title><bean:message bundle="NetworkApplicationResources" key="<%= NetworkConstants.JSP_RESULTS_TITLE %>"/></title>
  ...
  <bean:message bundle="NetworkApplicationResources" key="jsp.search.results.title"/>
</h2> 

<display:table id="elementSearch" style="width:100%" requestURI="SearchCommitNetworkAction.do" name="<%= searchListName %>" pagesize="10" export="true" sort="list" partialList="true" size="resultSize" decorator="com.hp.ov.activator.mwfm.servlet.inventory.inventorytree.taglib.displaytag.decorator.InventoryBuilderTableDecorator">

<%
        if ( NetworkId___hide == null || NetworkId___hide.equals("null") ) {
%>
      <display:column property="networkid" sortable="true" titleKey="NetworkApplicationResources:field.networkid.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Name___hide == null || Name___hide.equals("null") ) {
%>
      <display:column property="name" sortable="true" titleKey="NetworkApplicationResources:field.name.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Type___hide == null || Type___hide.equals("null") ) {
%>
      <display:column property="type" sortable="true" titleKey="NetworkApplicationResources:field.type.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( ASN___hide == null || ASN___hide.equals("null") ) {
%>
      <display:column property="asn" sortable="true" titleKey="NetworkApplicationResources:field.asn.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( Region___hide == null || Region___hide.equals("null") ) {
%>
      <display:column property="region" sortable="true" titleKey="NetworkApplicationResources:field.region.alias" headerClass="tableTitle" class="tableCell"/>
<%
        }
        if ( ParentNetworkId___hide == null || ParentNetworkId___hide.equals("null") ) {
%>
      <display:column property="parentnetworkid" sortable="true" titleKey="NetworkApplicationResources:field.parentnetworkid.alias" headerClass="tableTitle" class="tableCell"/>
<%
    }
%>
</display:table>
...
String backAction = "SearchFormNetworkAction.do?datasource=" + datasource + "&tab_name=" + tabName + "&vi=" + viParameter + "&view=" + viewParameter + "&ndid=" + ndidParameter;
%>
...
%>
    <input type="submit" value="<%= backMessage %>" onclick="window.document.NetworkForm.submit()"/>
  </html:form>
