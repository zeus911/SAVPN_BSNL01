...
String formAction = "/CreationCommitServiceMultiplexAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
<html>
  <head>
    ...
    {
      window.document.ServiceMultiplexForm.action = '/activator<%=moduleConfig%>/CreationFormServiceMultiplexAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_location_=' + focusthis;
      window.document.ServiceMultiplexForm.submit();
    }
    function performCommit()
    {
      window.document.ServiceMultiplexForm.action = '/activator<%=moduleConfig%>/CreationCommitServiceMultiplexAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.ServiceMultiplexForm.submit();
    }
    function init()
    {
...
com.hp.ov.activator.vpn.inventory.ServiceMultiplex beanServiceMultiplex = (com.hp.ov.activator.vpn.inventory.ServiceMultiplex) request.getAttribute(ServiceMultiplexConstants.SERVICEMULTIPLEX_BEAN);
            ...
            String ServiceMultiplexId = StringFacility.replaceAllByHTMLCharacter(beanServiceMultiplex.getServicemultiplexid());
                  ...
                  String Vendor = StringFacility.replaceAllByHTMLCharacter(beanServiceMultiplex.getVendor());
...
ArrayList VendorListOfValues = (ArrayList) request.getAttribute(ServiceMultiplexConstants.VENDOR_LIST_OF_VALUES);
      VendorLabel = StringFacility.replaceAllByHTMLCharacter(VendorLabel);
                  String CardType = StringFacility.replaceAllByHTMLCharacter(beanServiceMultiplex.getCardtype());
...
ArrayList CardTypeListOfValues = (ArrayList) request.getAttribute(ServiceMultiplexConstants.CARDTYPE_LIST_OF_VALUES);
      CardTypeLabel = StringFacility.replaceAllByHTMLCharacter(CardTypeLabel);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="ServiceMultiplexApplicationResources" key="jsp.creation.title"/>
</h2> 
