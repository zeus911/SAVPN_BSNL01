...
String formAction = "/UpdateCommitQoSProfileAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("qosprofilename") ;
}
...
<html>
  <head>
    ...
    {
      window.document.QoSProfileForm.action = '/activator<%=moduleConfig%>/UpdateFormQoSProfileAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.QoSProfileForm.submit();
    }
    function performCommit()
    {
      window.document.QoSProfileForm.action = '/activator<%=moduleConfig%>/UpdateCommitQoSProfileAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.QoSProfileForm.submit();
    }
    function init()
    {
...
if(beanQoSProfile==null)
   beanQoSProfile = (com.hp.ov.activator.vpn.inventory.QoSProfile) request.getSession().getAttribute(QoSProfileConstants.QOSPROFILE_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String CustomerId = beanQoSProfile.getCustomerid();
        
            
                            
            
                
                String QoSProfileName = beanQoSProfile.getQosprofilename();
        
            
                            
            
                
                String Prefix = beanQoSProfile.getPrefix();
        
            
                            
            
                
                String Layer = beanQoSProfile.getLayer();
        
            
                            
            
                
                String PEQoSProfileName = beanQoSProfile.getPeqosprofilename();
        
            
                            
            
                
                String Profilename_in = beanQoSProfile.getProfilename_in();
        
            
                            
            
                
                String Profilename_out = beanQoSProfile.getProfilename_out();
        
            
                            
            
                
                String Compliant = beanQoSProfile.getCompliant();
        
            
                            
            
                
                String Description = beanQoSProfile.getDescription();
        
            
                            
            
                
                String AddressFamily = beanQoSProfile.getAddressfamily();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="QoSProfileApplicationResources" key="jsp.update.title"/>
</h2> 
...
<script>
    allEvents = allEvents + "checkShowRulesCustomerId();";//default invoked when loading HTML
    function checkShowRulesCustomerId(){
          var CustomerIdPass = false ;
      if (/^\\S+$/.test(document.getElementsByName("customerid")[0].value)) {CustomerIdPass = true;}
            
    var controlTr = document.getElementsByName("customerid")[0];
          if (true && CustomerIdPass ){
      ...
      allEvents = allEvents + "addListener(document.getElementsByName('customerid')[0],'keyup',checkShowRulesCustomerId);";
   </script>   
    <script>
function doOnLoad()
{
  // hide field
                    document.getElementsByName("customerid")[0].parentNode.parentNode.style.display="none";
                                                                    // check hiding rules
  eval(allEvents);
}
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="qosprofilename" value="<%= QoSProfileName %>"/>
                                                        <html:text disabled="true" property="qosprofilename" size="24" value="<%= QoSProfileName %>"/>
                                                </table:cell>
              <table:cell>
