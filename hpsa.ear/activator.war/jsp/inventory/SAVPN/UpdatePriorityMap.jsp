...
String formAction = "/UpdateCommitPriorityMapAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("priorityname") ;
}
...
<html>
  <head>
    ...
    {
      window.document.PriorityMapForm.action = '/activator<%=moduleConfig%>/UpdateFormPriorityMapAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.PriorityMapForm.submit();
    }
    function performCommit()
    {
      window.document.PriorityMapForm.action = '/activator<%=moduleConfig%>/UpdateCommitPriorityMapAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.PriorityMapForm.submit();
    }
    function init()
    {
...
if(beanPriorityMap==null)
   beanPriorityMap = (com.hp.ov.activator.vpn.inventory.PriorityMap) request.getSession().getAttribute(PriorityMapConstants.PRIORITYMAP_BEAN);
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="PriorityMapApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
              ...
              <table:cell>
                                                      <html:hidden property="priorityname" value="<%= PriorityName %>"/>
                                                        <html:text disabled="true" property="priorityname" size="24" value="<%= PriorityName %>"/>
                                                </table:cell>
              <table:cell>
