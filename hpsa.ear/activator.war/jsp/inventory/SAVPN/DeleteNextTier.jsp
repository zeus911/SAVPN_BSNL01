...
<html>
  <head>
    <title><bean:message bundle="NextTierApplicationResources" key="<%= NextTierConstants.JSP_DELETE_TITLE %>"/></title>
  ...
  function performCommit() {
      window.document.NextTierForm.action = '/activator<%=moduleConfig%>/DeleteCommitNextTierAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>';
      window.document.NextTierForm.submit();
    }
  function init() {
      var confirm = new HPSAConfirm('Confirm','<bean:message bundle="NextTierApplicationResources" key="delete.confirm"/>', '<bean:message bundle="InventoryResources" key="confirm.ok_button.label"/>', '<bean:message bundle="InventoryResources" key="confirm.cancel_button.label"/>');
...
<%
com.hp.ov.activator.vpn.inventory.NextTier beanNextTier = (com.hp.ov.activator.vpn.inventory.NextTier) request.getAttribute(NextTierConstants.NEXTTIER_BEAN);
...
nfB.setMaximumFractionDigits(6);
                  ...
                  String OriginNetworkElementId = StringFacility.replaceAllByHTMLCharacter(beanNextTier.getOriginnetworkelementid());
                        
                      String NextTierNetworkElementId = StringFacility.replaceAllByHTMLCharacter(beanNextTier.getNexttiernetworkelementid());
                        
	  String neId = request.getParameter("neid");
	  
	  com.hp.ov.activator.vpn.inventory.PERouter beanPERouter = null;
	  com.hp.ov.activator.cr.inventory.Network routerNetwork = null;
	  ...
	  Connection con = null;
		try
		{
			...
			if (ds != null)
			{
				...
				beanPERouter = com.hp.ov.activator.vpn.inventory.PERouter.findByNetworkelementid(con,neId);
				routerNetwork = com.hp.ov.activator.cr.inventory.Network.findByNetworkid(con,beanPERouter.getNetworkid());
...
<h2 style="width:100%; text-align:center;">
    <bean:message bundle="NextTierApplicationResources" key="<%= NextTierConstants.JSP_DELETE_TITLE %>"/>
</h2> 
</center>
<%
...
%>
      ...
      <table:header>
        ...
        <table:cell>
          <bean:message bundle="InventoryResources" key="description.heading"/>
            ...
            <table:cell>
                            <%= NetworkId != null? routerNetwork.getName() : "" %>
                          </table:cell>
            <table:cell>
    ...
    <html:form action="/DeleteCommitNextTierAction.do">
    <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
                  <html:hidden property="nexttiernetworkelementid" value="<%= String.valueOf(NextTierNetworkElementId) %>"/>
                        <html:hidden property="originnetworkelementid" value="<%= String.valueOf(OriginNetworkElementId) %>"/>
              </html:form>
  </body>
</html>
