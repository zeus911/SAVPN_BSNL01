...
String tabName = (String) request.getParameter(L3FlowPointConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.L3FlowPointFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="L3FlowPointApplicationResources" key="<%= L3FlowPointConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.L3FlowPointFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitL3FlowPointAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.L3FlowPointFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(L3FlowPointConstants.USER) == null) {
  response.sendRedirect(L3FlowPointConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="L3FlowPointApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitL3FlowPointAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
