...
String tabName = (String) request.getParameter(Sh_PolicyMappingConstants.TAB_NAME);
...
<script>
function clearForm() {
var formObj = document.Sh_PolicyMappingFormSearch;
...
if (element.type == 'select-one') { element.selectedIndex = -1; }
  ...
  var checkfalse=false;
            ...
            if(checkfalse){
    alert("<bean:message bundle="Sh_PolicyMappingApplicationResources" key="<%= Sh_PolicyMappingConstants.JSP_SEARCH_NO_SHOW %>"/>");
  }else{
    window.document.Sh_PolicyMappingFormSearch.action = '/activator<%=moduleConfig%>/SearchCommitSh_PolicyMappingAction.do?datasource=<%= datasource %>&tab_name=<%= tabName %>&vi=<%= viParameter%>&view=<%= viewParameter %>&ndid=<%= ndidParameter %>';
    window.document.Sh_PolicyMappingFormSearch.submit();
  }
  }
</script>
...
if (session == null || session.getValue(Sh_PolicyMappingConstants.USER) == null) {
  response.sendRedirect(Sh_PolicyMappingConstants.NULL_SESSION);
  return;
}
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="Sh_PolicyMappingApplicationResources" key="jsp.search.title"/>
</h2> 
...
<% String searchFormAction = "/SearchCommitSh_PolicyMappingAction.do?datasource="+datasource+"&tab_name="+tabName+"&vi="+ viParameter+"&view="+ viewParameter +"&ndid="+ ndidParameter ;%>
