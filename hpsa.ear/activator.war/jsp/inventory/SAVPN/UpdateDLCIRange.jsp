...
String formAction = "/UpdateCommitDLCIRangeAction.do?datasource=" + datasource + "&rimid=" + rimid;
...
if ( pk == null || pk.equals("") ) {
  pk =    request.getParameter("dlcirangeid") + "||" + request.getParameter("usage") + "||" + request.getParameter("allocation") + "||" + request.getParameter("region") ;
}
...
<html>
  <head>
    ...
    {
      window.document.DLCIRangeForm.action = '/activator<%=moduleConfig%>/UpdateFormDLCIRangeAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>&_location_=' + focusthis;
      window.document.DLCIRangeForm.submit();
    }
    function performCommit()
    {
      window.document.DLCIRangeForm.action = '/activator<%=moduleConfig%>/UpdateCommitDLCIRangeAction.do?datasource=<%= datasource %>&rimid=<%= rimid %>&_pk_=<%=pk %>';
      window.document.DLCIRangeForm.submit();
    }
    function init()
    {
...
if(beanDLCIRange==null)
   beanDLCIRange = (com.hp.ov.activator.vpn.inventory.DLCIRange) request.getSession().getAttribute(DLCIRangeConstants.DLCIRANGE_BEAN);
...
nfB.setMaximumFractionDigits(6);
              ...
              String DLCIRangeID = beanDLCIRange.getDlcirangeid();
        
            
                            
            
                
                String Usage = beanDLCIRange.getUsage();
        
            
                            
            
                
                String Allocation = beanDLCIRange.getAllocation();
         ...
         EndValue = "";
                ...
                String Description = beanDLCIRange.getDescription();
        
            
                            
            
                
                String Region = beanDLCIRange.getRegion();
...
<h2 style="width:100%; text-align:center;">
  <bean:message bundle="DLCIRangeApplicationResources" key="jsp.update.title"/>
</h2> 
...
<input type="hidden" name="_update_commit_" value="true"/> 
    <table:table>
      ...
      <html:hidden property="refreshTreeRimid" value="<%= (String)request.getParameter(\"refreshTreeRimid\") %>"/>
                                                  ...
                                                  <html:hidden property="dlcirangeid" value="<%= DLCIRangeID %>"/>            
              ...
              <table:cell>
                                                      <html:hidden property="usage" value="<%= Usage %>"/>
                                                        <html:text disabled="true" property="usage" size="24" value="<%= Usage %>"/>
                                                </table:cell>
              <table:cell>
              ...
              <table:cell>
                                                      <html:hidden property="allocation" value="<%= Allocation %>"/>
                                                          <%
                    ...
                    <html:select disabled="true" property="allocation" value="<%= selValue %>" >
