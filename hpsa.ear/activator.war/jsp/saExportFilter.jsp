...
<%!
    //I18N strings
    final static String header          = com.hp.ov.activator.activatorUiNLS.MsgHandler.getString("688", "Statistical Export Filter");
    final static String select          = com.hp.ov.activator.activatorUiNLS.MsgHandler.getString("586", "Select");
    final static String textMsg         = com.hp.ov.activator.activatorUiNLS.MsgHandler.getString("689", "Export data in this range");
...
<form name="inputForm" action="/activator/jsp/saExportRedirect.jsp?formType=<%=formType %>" method="POST" onsubmit="return checkForm()">
<%
...
<%
  String selHost=statExpFilter.getHostName(), name;
...
<tr>
    ...
    <td nowrap title="<%=MessageFormat.format(beginDateTitle,new Object[]{df.toPattern()})%>"><input type="text" size="8" name="beginDate" 
        title="<%=MessageFormat.format(beginDateTitle,new Object[]{df.toPattern()})%>" id='inputBeginDate' value="<jsp:getProperty name="statExpFilter" property="beginDate" />"/>
    ...
    <input type="text" size="8" name="beginTime" title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>" value="<jsp:getProperty name="statExpFilter" property="beginTime" />"/></td>
</tr>
<tr>
    ...
    <td nowrap title="<%=MessageFormat.format(endDateTitle,new Object[]{df.toPattern()})%>">
        <input type="text" size="8" name="endDate" id='inputEndDate' title="<%=MessageFormat.format(endDateTitle,new Object[]{df.toPattern()})%>" value="<jsp:getProperty name="statExpFilter" property="endDate" />"/>
        <img src="/activator/images/date_select.gif" id="buttonEndDate" class="dateInputTriger"/>
    </td>
    <td class="filterTextTd" title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>"><%=time%>:</td>
    <td title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>">
        <input type="text" size="8" name="endTime" title="<%=MessageFormat.format(timeTitle,new Object[]{tf.toPattern()})%>" value="<jsp:getProperty name="statExpFilter" property="endTime" />"/></td>
</tr>
</table>
<br/>
<br/>
