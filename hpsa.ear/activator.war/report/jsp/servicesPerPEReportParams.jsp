...
<%--##############################################################################--%>
<%--                                                                              --%>               
<%-- $Source: /tmp/vpn/SA_VPN_SP/OpenView/ServiceActivator/solutions/SAVPN/UI/report/jsp/servicesPerPEReportParams.jsp,v $--%>
<%-- $Revision: 1.10 $                                                                 --%>
...
<%@page contentType="text/html;charset=UTF-8" import="com.hp.ov.activator.mwfm.*,
                ...
                com.hp.ov.activator.cr.inventory.NetworkElement,com.hp.ov.activator.cr.inventory.Region,
				com.hp.ov.activator.cr.inventory.Location,
				com.hp.ov.activator.inventory.SAVPN.SwitchConstants,
                java.sql.*, 
...
<%
    ...
    String link1 = "'servicesPerPEReportParams.jsp?";
    ...
    try {
        ...
        if(reg != null){
            ...
            if(loc != null){
               ...
               // pe = NetworkElement.findByLocation(connection, locationSelected);
...
<center>
    ...
    <h2 class="mainSubHeading">
        <center><bean:message bundle="InventoryResources" key="Report.ServicePerPE.title" /></center>
    </h2>
    <input type="hidden" name="reportType" value="servicePerPEReport"/>
    <table align="center" width=70% border=0 cellpadding=0>
        <tr>
            <td class="mainHeading" width=30% colspan='3'><bean:message bundle="InventoryResources" key="Report.ServicePerPE.select" /></td>
        </tr>
        <tr id="rpRow1" class="tableOddRow">
            <td class="tableCell">&nbsp;</td>
            <td class="tableCell" width=30%><B><bean:message bundle="InventoryResources" key="Report.ServicePerPE.Region" /></B></td>
            <td class="tableCell">
            <%if(regions.isEmpty()){
                enable=false;%>
            ...
            <td class="tableCell">&nbsp;</td>
            <td class="tableCell" width=30%><B><bean:message bundle="InventoryResources" key="Report.ServicePerPE.Location" /></B></td>
            <td class="tableCell">
            ...
            <td class="tableCell">&nbsp;</td>
            <td class="tableCell" width=30%><B><bean:message bundle="InventoryResources" key="Report.ServicePerPE.Routers" /></B></td>
            <td class="tableCell">
        ...
        <tr id="rpRow3" class="tableEvenRow">
            <td class="tableCell" colspan='3' align="right">
                <%if(enable==true){%>
                    <input type="submit" value="<bean:message bundle="InventoryResources" key="Report.ServicePerPE.Submit" />" name="B1">
                <%}else{%>
                    <input type="submit" value="<bean:message bundle="InventoryResources" key="Report.ServicePerPE.Submit" />" name="B1" disabled = "true">
                <%}%>
            </td>
        </tr>
    </div>
