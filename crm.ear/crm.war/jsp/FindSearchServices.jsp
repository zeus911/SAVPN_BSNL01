...
<%
  ...
  response.setHeader("Pragma","no-cache");
   ServiceForm ListServiceForm = (ServiceForm)request.getAttribute("SearchSiteSubmit");
	if(ListServiceForm == null)
	ListServiceForm = (ServiceForm)request.getAttribute("ServiceForm");
   ...
   String customerid = ListServiceForm.getCustomerid();
	String customername=   ListServiceForm.getCustomername();
	String serviceid=ListServiceForm.getServiceid();	
	String sitename=ListServiceForm.getPresname();
	String searchSite="searchSite";
...
<html:html locale="true">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="css/awfweb2.css">   
	 ...
	 function init(custid,pt,currpage,isnavigate,currentRs,lastRs,totalPages,strSort)
			 {
	    ...
	    parent.reload.setURL('/crm/SearchSiteSubmit.do?serviceid=<%= serviceid %>&customername=<%= customername %>&presname=<%= sitename %>&sort='+strSort+'&mv='+pt+'&currentPageNo='+currpage+'&navigate='+isnavigate+ '&currentRs='+currentRs+'&lastRs='+lastRs+'&totalPages='+totalPages);
...
<%
   ...
   nextStateMap.put("PE_CE_Modify_Partial","PE_CE_Undo_Modify_Sent");
	...
	try {
		 ...
		 Service service=ListServiceForm.getService();
		...
		{	
		//System.out.println("entra en parentServices======="+parentServices.length);
...
<body onmousemove="parent.reload.setYPos();" onUnLoad="tmp=parent.reload.bDoUpdate; parent.reload.stopReload('true'); parent.reload.bDoUpdate=tmp; window.status = '';" onLoad="init('<%=customerid%>','<%=pt%>','<%=cpage%>','<%="false"%>','<%=currentRs%>','<%=lastRs%>','<%=totalPages%>','<%=strSort%>');">
  ...
  <table width="100%" border="0">
      <tr>
        ...
        <td class="white">
			...
			<a href="/crm/SearchSiteSubmit.do?serviceid=<%= serviceid %>&customername=<%= customername %>&presname=<%= sitename %>&doResetReload=true&mv=<%="viewpageno"%>&currentPageNo=<%=cpage%>&viewPageNo=<%=vPageNo%>&sort=<%=strSort%>" target="main"><img src="images/refresh.gif" border="0" align="right" title="Refresh"></a>
		</td>
      </tr>
  </table>
</center>
			 ...
			 <body onmousemove="parent.reload.setYPos();" onUnLoad="tmp=parent.reload.bDoUpdate; parent.reload.stopReload('true'); parent.reload.bDoUpdate=tmp; window.status = '';" onLoad="init('<%=customerid%>','<%=pt%>','<%=cpage%>','<%="false"%>','<%=currentRs%>','<%=lastRs%>','<%=totalPages%>','<%=strSort%>');">
			  ...
			  <table width="100%" border="0">
				  <tr>
					...
					<td class="white"><h2 class="mainSubHeading"><center>
					<bean:message key="title.cust.service"/></center></h2>
					</td>
...
%>
        <p>  
		<html:form action="/SearchSiteSubmit">
...
String sortAsc = "<img src='images/up.gif' align='absmiddle'  border='0'/>";
 ...
 <table align="center" width="100%" border="0" cellpadding="2" cellspacing="1">
            ...
            <th class="center"> 
		    <font color = "#C0C0C0">
			<a href="/crm/ListAllServices.do?customerid=<%= customerid %>&doResetReload=true&sort=<%=currentSort%>&mv=<%="viewpageno"%>&currentPageNo=<%=cpage%>&viewPageNo=<%=vPageNo%>" style= color:white>
...
<%         
	...
	<%
		...
		for(int i=0;i<services.length;i++){	
			...
			String isParent = "N";
			final String foreignCustomerId = null;
			final String foreignCustomerName = null;
			...
			if(((services[i].getParentserviceid()==null) && (siteCount == 0)) && 
					   ((type.equals("layer2-VPWS"))||(type.equals("layer2-VPN"))))
				     { 
			...
			if((services[i].getParentserviceid()==null) && ((type.equals("layer3-VPN"))||(type.equals("layer2-VPWS"))||(type.equals("layer2-VPN")))) {
					isParent="Y";
 ...
 %>
	...
	<%                        } else {
		%>
								<select name="type0" id="subserv<%=services[i].getServiceid()%>" onchange="storevalue(this);">
	<%                            if (subServiceTypes != null) {
									for (int jj=0; jj<subServiceTypes.length; jj++) { %>
									  ...
									  <%/**PR 15763 Begin*/ 
										...
										if(subServicename != null && subServicename.equals(Utils.escapeXml(subServiceTypes[jj].getName()))){%>selected<%}  %>>
...
//jacqie - PR 15324
 ...
 <td class="<%=rowStyle%>" valign="middle"></td>
				...
				<a href="/crm/SearchSiteSubmit.do?serviceid=<%= serviceid %>&customername=<%= customername %>&presname=<%= sitename %>&sort=<%=strSort%>&customerid=<%= customerid %>&doResetReload=true&mv=<%="viewpageno"%>&currentPageNo=<%=cpage%>&viewPageNo=<%=vPageNo%>&selected_serviceid=<%=currentService.getServiceid()%>&selected_att=<%=selected_att%>"><img src="/crm/images/<%=treeState%>.gif" border="0" ></a>
...
%>
     ...
     function callviewByPageNo()
	{
		...
		var page = "/crm/ListAllServices.do?mv=viewpageno&doResetReload=true&customerid="+customerid+"&currentPageNo="+currentPageNo+"&viewPageNo="+viewPageNo+"&sort="+sort;
        self.location.href = page;
		}
	}
...
//PR 15763 Begin
	function storevalue(serviceList) {
	   ...
	   var customerid='<%=customerid%>';
		...
		var page = "/crm/ListAllServices.do?mv=viewpageno&doResetReload=true&customerid="+customerid+"&currentPageNo="+currentPageNo+"&viewPageNo="+document.forms[0].txtviewPageNo.value+"&sort="+sort+"&selValue="+selValue+"&selId="+selId;
		//alert(page);
        self.location.href = page;
