...
###############################################################################
#
# $Source: /tmp/vpn/SA_VPN_SP/jboss/server/default/deploy/crmportal.sar/crm.war/forms/AddStaticRouteslayer3-Attachment.xsl,v $
# $Revision: 1.7 $
...
<header>
    <Service_request Response="{$request_synchronisation}" Mode="{$skip_activation}" Operator="{$operator}">
      ...
      <Activation_name>modify_StaticRoutes</Activation_name>
    ...
    <Service Service_id="{$serviceid}">
        <SiteAttachment Attachment_type="L3VPN">
            <L3Resources Attachment_Address_family="{$AddressFamily}">
                <Routing Protocol="STATIC" Address_type="{$AddressFamily}">
