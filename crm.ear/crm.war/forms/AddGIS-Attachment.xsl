...
###############################################################################
#
# $Source: /tmp/vpn/SA_VPN_SP/jboss/server/default/deploy/crmportal.sar/crm.war/forms/GISSite-Attachment.xsl,v $
# $Revision: 1.11 $
...
-->
	...
	<xsl:param name="IPNetAddr"/>
	<xsl:param name="PE1Addr"/>
	<xsl:param name="CE1Addr"/>
	<xsl:param name="IPNetmask"/>
	...
	<xsl:param name="PERouter"/>
	<xsl:param name="PEInterface"/>
	...
	<xsl:template name="prefixstrsplit">
		<xsl:param name="ipprefix"/>
		<xsl:choose>
			<xsl:when test="contains($ipprefix,',')">
				<xsl:element name="Prefix_route">
					<xsl:value-of select="substring-before($ipprefix,',')"/>
				</xsl:element>
				<xsl:call-template name="prefixstrsplit">
					<xsl:with-param name="ipprefix" select="substring-after($ipprefix,',')"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:element name="Prefix_route">
		...
		<msg msg_id="{$messageid}">
			...
			<body>
				<Service Service_id="{$serviceid}">
					...
					<SiteAttachment Attachment_type="GISVPN" Attachment_role="{$Attachmenttype}">
						<Attachment_name>
						...
						<xsl:if test="$PERouter!='' and $PEInterface!=''">
							<Attachment_tp Edge="PE">
								<Network_element>
									<NE_name>
										<xsl:value-of select="$PERouter"/>
									</NE_name>
								</Network_element>
								<Port>
									<Port_name>
										<xsl:value-of select="$PEInterface"/>
									</Port_name>
									<Media_type><Ethernet Encapsulation="none"/></Media_type>
								</Port>
							</Attachment_tp>
						</xsl:if>
						<L3Resources Attachment_Address_family="{$AddressFamily}">
							<Routing Protocol="{$RoutingProtocol}" Address_type="{$AddressFamily}">
								<xsl:choose>
									...
									<xsl:when test="$RoutingProtocol='BGP'">
										...
										<xsl:call-template name="prefixstrsplit">
											<xsl:with-param name="ipprefix" select="$PREFIX_Routes"/>
										</xsl:call-template>
									</xsl:when>
									<xsl:when test="$RoutingProtocol='STATIC'">
										<xsl:call-template name="strsplit">
											<xsl:with-param name="src" select="$STATIC_Routes"/>
										</xsl:call-template>
									</xsl:when>
								</xsl:choose>
							</Routing>
							<AddressPool Pool_name="{$AddressPool}" Address_type="{$AddressFamily}" >
								 <IPNet>
							   <IPAddress><xsl:value-of select="$IPNetAddr"/></IPAddress>
							   <PE1_IPAddr></PE1_IPAddr>
							   <CE1_IPAddr></CE1_IPAddr>
							   <IPNetmask></IPNetmask>
							</IPNet>
							</AddressPool>
