...
###############################################################################
#
# $Source: /tmp/vpn/SA_VPN_SP/jboss/server/default/deploy/crmportal.sar/crm.war/forms/ModifyRateLimitInterfacelayer3-Attachment.xsl,v $
# $Revision: 1.8 $
...
<xsl:param name="Source_Interface"/>
...
<header>
    <Service_request Response="{$request_synchronisation}" Mode="{$skip_activation}" Operator="{$operator}">
      ...
      <Activation_name>modify_RLInterface</Activation_name>
  ...
  <Service Service_id="{$serviceid}">
    <SiteAttachment Attachment_type="L3VPN">
	  ...
	  <Source>
		<Interface><xsl:value-of select="$Source_Interface"/></Interface>
	  </Source>
	  <Target>
