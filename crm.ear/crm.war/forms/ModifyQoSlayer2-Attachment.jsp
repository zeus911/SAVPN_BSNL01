...
<%@ page import="com.hp.ov.activator.crmportal.common.*,
                 com.hp.ov.activator.crmportal.action.*,
                 com.hp.ov.activator.crmportal.bean.*,java.util.HashMap,java.util.HashMap,java.util.*, java.io.*, java.text.*, java.net.*"%>
...
<%  
   ...
   String customer_id = serviceForm.getCustomerid();
	...
	String failureLink = "/crm/ListAllServices.do?customerid="+customer_id+"&doResetReload=true&mv=null";
    CAR[] rateLimits = null;

	int rowCounter= Integer.parseInt(request.getParameter("rowCounter"));

    String rateLimit = (String)request.getAttribute("SP_CAR");
    if(rateLimit == null)
      rateLimit = (String) serviceParameters.get("CAR");

    String link_part1 = "'/crm/ModifyService.do?type=layer2-Attachment&serviceid=" + serviceid +
    ...
    try{
        ...
        for(int i = 0; i < mappingsLength; i++){
            ...
            if(request.getAttribute(spParamName) == null && serviceParameters.get("QOS_CLASS_"+i+"_PERCENT") != null) {
...
%>
    <script>
    alert('<bean:message key="js.exp.error" /><%= ex.getMessage () %>.');
