...
<%
	...
	String vpnid = serviceForm.getServiceid();
	...
	try
	{
		...
		if (ds != null)
		{
			...
			String query1 = "select distinct usagemode from v_flowpoint where attachmentid IN (select fp.attachmentid from v_flowpoint fp, v_vpnfpmembership vf where fp.terminationpointid=vf.flowpointid and vf.vpnid=?)";
			
			statePstmt1 = con.prepareStatement(query1);
			statePstmt1.setString(1, vpnid);
			resultSet1 = statePstmt1.executeQuery();
			
			while(resultSet1.next())
			{
				currentMode = resultSet1.getString(1);
			}

			// Find out if there are Service LSPs created as a consequence of the creation of this attachment
			String query2 = "select lsp.lspid from v_flowpoint fp, v_vpnfpmembership vf, cr_terminationpoint tp, v_lsp lsp, v_lspvpnmembership lv "
