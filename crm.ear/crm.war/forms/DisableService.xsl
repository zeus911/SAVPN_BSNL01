...
###############################################################################
#
# $Source: /tmp/vpn/SA_VPN_SP/jboss/server/default/deploy/crmportal.sar/crm.war/forms/DisableService.xsl,v $
# $Revision: 1.7 $
...
<msg msg_id="{$messageid}">
  <header>
    <Service_request Response="{$request_synchronisation}" Mode="{$skip_activation}" Operator="{$operator}">
      ...
      <Activation_name>modify_AdminState</Activation_name>
      <Service_name><xsl:value-of select="$servicetype"/></Service_name>
    </Service_request>
  </header>
<body>
	<Service Service_id="{$serviceid}">
    	<SiteAttachment Attachment_type="{$Attachmenttype}" Attachment_state="{$EnableState}" />
    </Service>
</body>
</msg>
</xsl:template>
