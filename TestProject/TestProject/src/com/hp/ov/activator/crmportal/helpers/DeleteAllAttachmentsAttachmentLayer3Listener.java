...
public class DeleteAllAttachmentsAttachmentLayer3Listener implements StateListener
{
    ...
    //here count the attachments associated with this site 
     ...
     if(attachments != null){
           logger.debug("DeleteAllAttachmentsAttachmentLayer3Listener:: Site_"+siteService.getServiceid()+"'s attachments.length is "+attachments.length);
     }else {
           logger.debug("DeleteAllAttachmentsAttachmentLayer3Listener:: Site_"+siteService.getServiceid()+" is null");
     }          
     
     if(attachments != null)
	 {    	 
		 // If there are no protection attachment delete the site
		 if (attachments.length == 1 )
		 {
			...
			sender.Init();
			sender.Send();
			logger.debug("DeleteAllAttachmentsAttachmentLayer3Listener:: Delete site_"+siteService.getServiceid()+" is sent");
		  }
		   ...
		   String vpnid = ServiceUtils.getServiceParam(connection, serviceId, "vpnserviceid");
			...
			ServiceUtils.saveOrUpdateParameter(connection, vpnService.getServiceid(), Constants.PARAMETER_LAST_COMMIT, DeleteAllAttachmentsVPNLayer3Listener.class.getName());
			...
			catch (IOException e) 
			{
				...
				{
					logger.error("Exception in DeleteAllAttachmentsAttachmentLayer3Listener listener" + ex);
				}
			}
	 ...
	 else if (state.indexOf("Failure") > -1)
	{
       ...
       //set the protection to initial attachment
        Service[] protection = Service.findByParentserviceid(connection, attachmentService.getParentserviceid(), 
           "type='layer3-Protection' and serviceid in (select serviceid from crm_serviceparam where attribute='attachmentid' and value='"+attachmentService.getServiceid()+"')");
       if(protection != null && protection.length > 0){
    	 ...
    	 logger.debug("DeleteAllAttachmentsAttachmentLayer3Listener::Site has more than one attachments So Delete site request is not sent");
     }
    		...
    		logger.error("DeleteAllAttachmentsAttachmentLayer3Listener:::Exception in DeleteAllAttachmentsAttachmentLayer3Listener"+ex);
    }
