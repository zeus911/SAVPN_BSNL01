...
public class CreateGISSiteListener  implements StateListener{
	  ...
	  public int proccedState(Connection connection, String serviceId, String state, String data) throws Exception {
	    ...
	    //if (PortalSyncListener.DEBUG) {
		  logger.debug("Inside StateListener for GIS site");
	...
	//state based on ACTIVATION SCOPE
	   ...
	   //logger.debug("StateListener for layer3site: state set is  "+attachment_service.getState());
	    ...
	    String vpnserviceid=ServiceUtils.getServiceParam(connection, serviceId, "gisvpnserviceid");
	    ...
	    attachment_service.setPresname(site.getPresname()+"GIS-Attachment");
	    ...
	    attachment_service.setType("GIS-Attachment");

	        attachment_service.store(connection);


	        logger.debug("GisSite listener:::::::::::::Stored the attachment service bean");
	    ...
	    allParameters.put("presname",site.getPresname()+"GIS-Attachment");
	  ...
	  //set the lastcommit action to CreateL3VPNStateListener
	    ServiceUtils.saveOrUpdateParameter(connection, attachment_service_id, Constants.PARAMETER_LAST_COMMIT, CreateGISVPNStateListener.class.getName());
	      ...
	      sender.Send();
	        ...
	        try{
	          ...
	          ServiceUtils.saveOrUpdateParameter(connection, attachment_service_id,
	    		...
	    		logger.error("Exception in gisSite listener"+ex);
	    }
