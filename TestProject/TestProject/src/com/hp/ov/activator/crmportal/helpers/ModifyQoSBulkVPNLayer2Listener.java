...
public class ModifyQoSBulkVPNLayer2Listener implements StateListener
{
	...
	public int proccedState(Connection connection, String serviceId, String state, String data) throws Exception 
	{
		...
		try 
		{
			...
			// Find the first attachment that does not match the CAR and QOS of the VPN
			for (int i=0; i < attachments.length; i++) 
			{
				try
				{
					...
					if (!(selectedCAR.equals(attachmentCAR) && selectedQoS.equals(attachmentQoS)))
					{
			...
			if (attachment != null)
			{			
				if ("layer2-Attachment".equals(attachment.getType()))
				{
					...
					ServiceUtils.saveOrUpdateParameter(connection, attachment.getServiceid(), Constants.PARAMETER_LAST_COMMIT, ModifyQoSBulkAttachmentLayer2Listener.class.getName());
					...
					catch (IOException e) 
					{
						...
						{
							logger.error("Exception in ModifyQoSBulkVPNLayer2Listener listener" + ex);
						}
					}
				} // if layer2 attachment
			...
			else
			{
				...
				else
				{
					...
					// Save listener for QoS Bulk operation
					ServiceUtils.saveOrUpdateParameter(connection, vpnService.getServiceid(), Constants.PARAMETER_LAST_COMMIT, ModifyQoSBulkVPNLayer2Listener.class.getName());
